...
⚠️ **Discord ID の登録はこの画面で完結させた。他ページへ案内していない。**
案内先を3回間違えてから辿り着いた結論（すべて教訓21の型）:

| 案内先の候補 | 何が起きるか |
|---|---|
| 「Store Operations → Management Channel → Discord IDs」 | **そんな経路は存在しない**（最初に私が書いた文言） |
| `/admin/discord-alerts`（メニューにある） | 書くのは `discord_alert_recipients`＝**店舗別の遅刻アラート名簿**。足すと頼まれていない勤怠DMが届き始める |
| `/admin/management/assignments`（正しい表を書く） | **NavBar に無い**。さらに致命的なのは、ID入力欄が**例外タイプの担当者にしか出ない** — Mariano と Aliana は担当者ではないので、**この画面では直せない** |

→ **バナーに名前ごとの入力欄と Save を置いた**（`PUT /api/admin/management/channel-discord`、
任意の staff_name を受ける・数字以外は400で拒否）。保存後に一覧を再取得するので、
**直した名前がバナーから消える** — 消えなければ保存できていない。

⚠️ **リンクを置いて満足しかけた。** リンク先が開けることは確認したのに、
**その画面がその人を登録できるかを確認していなかった。**
「画面が存在する」と「その画面でこの作業ができる」は別（教訓56）。
