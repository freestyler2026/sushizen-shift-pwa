...
## ✅ Prep Time: OCRがプロンプトの出力例を記録していた（2026-09-01・修正済み）

**全記録の20%（1,757件）が写真と無関係に生成されていた。**

```python
# 修正前のプロンプト末尾
{"has_receipt":true,...,"order_no":"GF-192","ordered_at":"7:59 PM",
 "ready_by":"8:20 PM","prep_minutes":21,"confidence":"high"}
```
レシートが読めない写真では `has_receipt:false` ではなく**この例をそのまま返していた**。
→ `GF-192 / 7:59 PM→8:20 PM / 21分` が **8店舗・37日**。投稿はすべて別々のDiscordメッセージで投稿者は実在スタッフ。
**GrabFoodはドバイに無いのに967件がドバイ。**

**気づけなかった理由**: 21分・スコア80は平均そのもので、集計値を見る限り異常に見えない。
発覚は「同一時刻の行が並んでいる」という画面の見た目から。

**修正**:
- プロンプトの例を `"<as printed>"` 等のプレースホルダに変更＋「推測するな」を明記
- サーバ側で例と一致する応答・`<`で始まる値を弾く（プロンプトだけでは次の漏洩を止められない）
- 既存分は**削除せず集計から除外**。`excluded_reason` を付けて一覧には残し、除外件数をAPIに含める
- 検知は `order_no='GF-192'` **OR** 時刻ペア一致。AND だと片方だけ保持した534件を取りこぼす

**下限も追加**: `PREP_TIME_FLOOR_MIN`（既定3分、デプロイ不要で変更可）。
10分以下は全て満点になる採点なので1〜5分の記録は全件スコア100。分布に切れ目が無く統計では線を引けないため、
「3分未満は受注と完了がほぼ同時＝ボタン操作」という厨房側の理屈で決めた。除外45件。

### Taft の遅さ（偽データ除去後の再分析）

**偽データを除いてもTaftは最も遅い。** 22.2分（他店 20.1〜21.1）、30分超が87件（7%）。

| 時間 | 件数 | 平均 | 30分超 | 他マニラ店との差 |
|---|---|---|---|---|
| 11時 | 28 | 26.6分 | 14% | **+7.7** |
| 13時 | 47 | 25.2分 | 9% | **+4.8** |
| 21時 | 38 | 25.2分 | **18%** | **+4.5** |
| 20時 | 88 | 24.7分 | 11% | +2.0 |
| **19時（最繁忙 387件）** | 387 | 22.6分 | 8% | +2.2 |

**最も忙しい19時は最悪ではない。遅いのは前後（11時・13時・20〜21時）。**
ピークは人が揃っており、その前後で足りていない可能性。**人員配置の形の問題**として見るべき。

曜日は木21件・金22件が突出（日5件）。最悪日は 2026-08-28（11件）。

⚠️ **未対応: ドバイの `grabfood` が3,498件。** GrabFoodはUAEに存在しない。
アグリゲーター名の誤りで、所要時間自体は正しい可能性が高いため未着手。要判断。

---

## ✅ 全プラットフォーム欠測ゼロ達成（2026-09-01）
...
| 対象 | 原因 | 対応 |
|---|---|---|
| Grab PAR/TAFT/CUB | セッション切れ（6日） | 再ログイン後に取込。3店舗とも 9/2 まで |
| Smiles ARJ/JLT | **欠測ではなく8月の注文0件** | nil月を記録する仕組みを追加 |
| Talabat 3チェーン | **セッションではなく実行頻度** | 月次→日次に変更 |

### ⚠️ Talabat の教訓: 「セッション切れ」に見えて違った
`talabat-payout-extract.yml` は**5日に1回/月**なのに、Talabatは**3〜4日ごと**に入金する。
つまり記録は常に最大1ヶ月遅れ。9/1時点で 8/21・8/24 の入金がポータルに存在したまま未取得だった。
**取得頻度の不一致とセッション切れは同じ「沈黙」を生むが、ログインし直して直るのは片方だけ。**
→ 日次・ローリング45日に変更（`payout_id` でupsertなので重複しない）。

### ⚠️ Grab の教訓: セッションファイル名と中身がずれる
`paranaque-session.json` に **Quezon City のログイン**が入っていた（ブラウザに前のセッションが残っていたため）。
`fetchDailySummary` は `merchant_group_id` で問い合わせるため**セッションが「誰の金か」を決め、
`GRAB_STORE_CODE` は「何と呼ぶか」を決めるだけ**。照合が無く、私は実際に
Cubao の入金を PAR として8日分取り込んだ（バックアップ `_grab_par_backup_20260901` を取って取消済み）。
→ `assertSessionMatchesStore()` を追加。書き込む前に停止する。
⚠️ **店舗名は表示名で照合しない。** Grabは Cubao を "Sushizen Japanese Restaurant - Quezon City" と呼ぶ。
識別語（paranaque / taft / quezon・cubao）で判定する。

### 暫定値の上方修正（Grabが後から確定値に差し替えたもの・全て未確認行）
```
PAR   8/25営業分  19,986.86 → 22,796.36  (+2,809.50)
CUB   8/25営業分  26,347.47 → 28,817.52  (+2,470.05)
TAFT  8/25営業分  32,216.14 → 33,412.54  (+1,196.40)
```

### 取込額
```
Grab   PAR 213,796.33 / TAFT 308,537.71 / CUB 223,335.82 (PHP・8日分ずつ)
Talabat  SZ 61,297.03 / RZ 6,762.78 / AVS 1,192.26 (AED・8期間ずつ)
Smiles   5支店すべて 8/31 まで（ARJ・JLT は nil月）
```

### ⏳ 残課題
- **JJAD (New — All 4 locations) チェーンが入金0件。** 新規契約エンティティで、まだ入金が発生していないか、
  取引が別チェーンに入っている可能性。次回入金時に確認する
- `get-net-payouts.js` のコメントは `694540 = Ramen ZEN` / `673913 = J-Japanese` だが、
  DBは `673913 = ramen_zen`。ブランド名の対応を実データで確定させること
...
### ① 重複4行を削除（オーナー指示）
バックアップ `_ar_payouts_backup_20260901`（61行）を取ってから削除。
`PAR` 表記の4行を削除し `FP_PARANAQUE` を残した（他90行と3店舗すべての規約）。
**重複グループ 0件。PHP 40,654.22 の二重計上を解消。**

### ② Talabat 店舗コードを統一（オーナー指示）
```
SZ  + 671526  -> TALABAT_SZ    16行 2025-09-30..2026-08-17
RZ  + 673913  -> TALABAT_RZ    16行
AVS + 698589  -> TALABAT_AVS   16行
```
時間の重複ゼロ＝同一支払先の連続履歴だった。命名は他のドバイ系の規約に合わせた。
取込時の正規化は `app/talabat_codes.py`（DBだけ直すと外部スクリプトが元に戻す）。

⚠️ **8/22の9行は入金ではなく日次売上**（`raw_data.data_type='gross_sales'`、AED 8,025.20）。
ドバイの日次P&Lと曜日別重みが `store_code` で読んでいるので**削除・改名しない**。
AR系のクエリから除外しただけ（db.pyの8箇所は元から除外していた）。
→ ドバイ Not checked 125→116件。

欠測検知も「判定不可」から **「8/17以降ゼロ・15日・通常3日周期・約5件欠測」** に変わった。
周期変更を捕まえるため第2規則を追加: **直近間隔の3倍（最低7日）**。

### ③ 自動取込
**Grab: 3店舗すべてセッション期限切れ**（ドライランで実証）。**再ログインが必要 — 私にはできません。**
```bash
node scripts/grab/setup-session.js paranaque   # taft / qc も同様
```
🔴 **CIが嘘をついていた**: `get-payouts.js` が `SESSION_EXPIRED` を表示した上で `process.exit(0)`。
**6日間、毎朝グリーンのチェックが出続けていた。** Careem・Keeta も同じ。
→ 3つとも `exit(1)` に修正。Talabatだけは元から webhook に sentinel を送っていた。

**Smiles: 毎回ログインし直す設計（期限切れなし）** → `gh workflow run smiles-dubai-payout.yml -f target_month=2026-08` で実行。
...
### ⚠️ 訂正: 「CKシートで削除→Publish」は**やってはいけない**

実データを見ると **Francis の9月は31日中30日がCKシートに書かれている**。
ただし各セルの `branch_code` は TAFT / PAR / CK と分かれており、
**CKシートは「彼のローテーション表」として使われている**（実際にCK勤務なのは 9/9 の1日だけ）。

```
sheet=CK  row_branch=TAFT   9/1,9/2,9/10..9/13,9/15,9/16,9/24..9/27,9/29,9/30
sheet=CK  row_branch=PAR    9/3..9/6,9/8,9/17..9/20,9/22,9/23
sheet=CK  row_branch=CK     9/9 のみ
sheet=TAFT row_branch=TAFT  9/1 のみ ← Aliana が別途作成。9/1 が二重になった原因
```

**CKシートから彼を消せば9月が丸ごと消える。** 実際に消すべきは **TAFTシートの 9/1 の1行だけ**。

→ 対策として `deleteStaffFromGrid` に**他支店セルの警告**を追加。
行のセルが現在のシート以外の支店を持つ場合、
「これは CK の勤務ではありません。TAFT, PAR の勤務が含まれます」と出してから確認する。
「31件が消えます」という件数だけでは、それが誰かの1ヶ月分だと伝わらない。

⚠️ **非表示機能を Francis の CK シートに使ってはいけない**（月全体が見えなくなる）。
非表示が有効なのは「そのシートに1〜2件だけ紛れ込んだ人」。
...
**入力の出所と説明文に4つの重大な問題**があった。

### 🔴 1. 8月の人件費と経費は「7月の数字」だった

```
dubai_monthly_labor に存在するのは 2026-07 の1ヶ月のみ
get_dubai_monthly_labor() は 12ヶ月遡ってフォールバック
→ 8月も9月も 7月の 214,566 を使用

mgmt_overhead も 2026-07 が最後
→ 両市とも8月・9月は7月の額を引き継ぎ
```
**8月のプライムコスト 86.9%「危険水準・至急確認」は、半分が7月の給与だった。**
経費は小さなキャプションで「2026-07 から引き継ぎ」と出ていたが、**人件費は何の表示も無し**
（`推定` バッジは `estimated_shifts` のときだけ）。
`overhead_missing` は引き継ぎ値が見つかると **False** になるので、未入力バナーも出ない。

→ `carried_inputs` を summary に追加。**画面に専用バナー**（どの項目がどの月の数字か）。
→ **引き継ぎ値を含む比率は critical にしない**（warning へ降格し、文言に「人件費は2026-07の数値」を付記）。

### 🔴 2. 「食材費は一致します」という説明が誤りだった

```
旧説明: 日次P&Lタブは食材費を同じ仕入実績から日割りするため、食材費は一致します

実際:   日次P&Lの食材費 = その日の売上 × 月の原価率
        両市とも 日次cogs/売上 が 月次food/売上 と小数第4位まで一致
```
ドバイが一致して見えたのは**売上の出所が同じ（入金）だから**。
マニラは日次売上がPOS基準で16%低く、**食材費も16%低く出る**（1,770,238 vs 2,102,233＝差 33万ペソ）。
**「一致するはず」と書かれた項目で33万の差**を見た人は、不整合を疑って調査に入る。

**人件費も出所が違う**（当ページ=給与ワークブック / 日次=給与明細、ドバイ8月 214,566 vs 227,732）が、
説明文に一言も無かった。→ 説明文を実態どおりに書き換え。

### 🟠 3. 為替レートが一度も設定されていない

`mgmt_fx_rates` は**0行**。全月が既定値 AED 40.5 / PHP 2.55 で計算されている。
設定画面は「Applies to 2026-09 only」と月別のように見せるため、
**「設定した40.50」と「誰も設定していない40.50」が区別できなかった。**
全社P&Lは全額この換算に依存する。→ `fx_defaulted` を返し、画面に「未設定（初期値のまま）」を表示。

### ✅ 検算して正しかったもの

```
円換算      dubai 463,917 × 40.5 = 18,788,638 ✓ / manila 5,684,383 × 2.55 = 14,495,177 ✓
全社合計    revenue/food/labor/overhead/prime/total すべて dubai+manila と一致 ✓
比率        food/prime とも 各都市・全社で再計算値と一致 ✓
恒等式      prime = food + labor、total = prime + overhead ✓
日次P&L     売上・経費はドバイで月次と一致（マニラの売上差は仕様どおり開示済み）
```

⚠️ **8月の全社営業利益 −4,415,042円は、ドバイの7月人件費と両市の7月経費を含んだ数字。**
当月分を登録するまで、この赤字額は確定値として扱えない。

---

## ✅ 「売上が未設定」の原因究明と再発防止（2026-09-01・本番反映済み）

### 原因: どちらも「月が始まったばかり」で、不具合ではなかった

| 都市 | 売上の出所 | 到着までの実際の遅れ |
|---|---|---|
| ドバイ | `ar_payouts`（アグリゲーター入金・手数料控除後） | **1〜2週間**。9/1時点で noon の最新取込は period_end 8/22 |
| マニラ | `manila_daily_sales`（日次手入力） | **翌朝**。8/31分は 9/1 06:20 に入力済み |

**9/1に9月の売上が無いのは当然。** しかし警告は月が変わった瞬間から出るため、
**毎月2週間ほど鳴りっぱなし**になっていた。原価率の閾値には
「常に鳴るアラートは何も伝えない」というコメントが既に書かれており、**同じ判断が売上側に無かった。**

さらに**ドバイは推定値で埋まっているのに「データがありません」と出ていた**（画面下部には
「実績 AED 0 ＋ 推定 AED 20,414」と表示されている）。これも矛盾。

### 修正: 3つに分ける

```python
ARRIVAL_LAG = {"dubai": 14, "manila": 2}   # 実際の挙動から
```

| 状態 | 出るもの |
|---|---|
| 推定値で埋まっている | 🔵 info「売上は推定値です」＋ 補った金額と理由 |
| 遅れの範囲内 | 🔵 info「売上はこれからです」＋ 何日経過・いつ届くか |
| **遅れを超えた／過去の月** | 🟠 warning「売上が未設定です」＋ **N日経過（通常M日）** |

**再発防止はこの3つ目。** 本当に取込が止まれば、ドバイは14日、マニラは2日で警告が出る。
マニラは別途 `manila_daily_sales_gaps()`（本日実装・worker で毎朝実行）が日単位の欠落も検知する。

### 検証

```
2026-09（当月1日目）  info 3件のみ。warning/critical ゼロ
2026-08（満月）       食材費 高水準 / プライムコスト 危険水準 / 売上減少 / 発注金額異常 ← 従来どおり
2026-10（データ無し・当月でない） warning revenue_missing ×2 ← 本当の欠落は鳴る
2025-11（実データあり）           revenue系アラート ゼロ
```

⚠️ **マニラの推定（`estimate_manila_month_revenue`）は `revenue > 0` が条件**。
過少計上の補正用なので、ゼロの月には効かない。ドバイのような月初の穴埋めはマニラには無い。

---

## ✅ 管理会計ページの調査と修正（2026-09-01・本番反映済み）

### 見つかった問題

**1. 矛盾するアラートが同時に出ていた**
```
🟠 ドバイ — 売上が未設定です  「原価率を算出できません」
🔴 ドバイ — 食材費 危険水準    「52.2% が基準48%超。至急確認してください」
🔴 ドバイ — プライムコスト 危険水準 87.3%
```
**「算出できない」と「52.2%だから至急」が3行差で並んでいた。**

**2. 1日分のデータで赤アラートを出していた**
```
days_covered=1 / days_in_month=30
revenue = 20,414（全額が推定値。実績は 0）
food_cost = 10,662（その月の仕入額。消費額ではない）
→ 52.2%
8月の実績（満月）は 40.6%
```
**食材費は仕入額なので、1回の納品が分子に丸ごと乗り、分母は1日分。** 跳ね上がって当然。
`partial_month=True` というフラグは**既に存在していて、アラート側が読んでいなかった**。
売上減少チェックは同じ理由で期間を揃える処理を持っており（コメントに経緯も記載）、
**原価率だけがその対象外だった。**

**3. `cutoff_day` が「当月に売上行が無い」とき月の日数にフォールバック**
→ 開始1日目の月を**完了した月として扱い、固定費を按分しない**。
```
マニラ 2026-09（1日目）  人件費 1,508,941 ＋ 固定費 1,280,808 ／ 売上 0
                        = 279万の赤字として表示されていた
修正後                   人件費 7,100 ＋ 固定費 42,694（1日分に按分）
```
...
| 対象 | 変更 |
|---|---|
| `cutoff_day` | 当月は**経過日数を下限**に。売上行が無くても満月扱いしない |
| 原価率アラート | **7日未満、または売上が推定のみ**なら critical/warning を出さない |
| 同上 | 代わりに `info`「原価率はまだ判断できません」で**参考値と理由を表示**（数字は隠さない） |
| 同上 | 部分月で発報する場合は文言に「（N/M日時点）」を付ける |
| UI | `info` は**青**で表示（型は warning/critical のみだったのでオレンジ＝警告に見えていた） |
| Manager Inbox | `info` は**送らない**（`severity_map` に無く、送ると黄色タスクとして溜まる） |
| 部分月バナー | 「30日のうち0日」→「1日経過して0日分（月全体は30日）」 |

**8月・7月のアラートは変化なし**（満月なので従来どおり発報）を確認済み。
...
SSSは `monthly_gross=22401.37` のみ、PhilHealthは `basic=21565.00` のみ、
**Pag-IBIGとBIRはnoteが空**で、入力値は分かっても金額になる過程が分からなかった。

```
SSS_EE       MSC 22,500 (bracket 22,250-22,750); regular SS on the first 20,000
             x 5% = 1,000.00; 50% this cut-off = 500.00 [monthly gross 22,401.37]
SSS_WISP_EE  MSC 22,500; MPF/WISP layer on the 2,500 above 20,000
             x 5% = 125.00; 50% this cut-off = 62.50
PHILHEALTH   basic 21,565.00 x 5% = 1,078.25, employee half = 539.13; 50% = 269.57
PAGIBIG      base 10,000.00 (capped at 10,000) x 2% = 200.00; 50% = 100.00
BIR          taxable = gross 45,000.00 - SSS/PH/HDMF 3,075.00 = 41,925.00/month;
             x12 = 503,100.00/year; bracket over 400,000: 22,500.00 + 20% of the
             excess = 43,120.00/year; /12 = 3,593.33/month
```

⚠️ **最初の実装はMSC 20,000超で計算が合わなかった。** 「MSC 22,500 × 5% = 1,125.00」を
1,000.00 の行に書いており、残り125.00がWISP行にあることを説明していなかった。
**従業員負担5%は2層（20,000までの通常SS ＋ 超過分のMPF/WISP）で、明細も2行**なので、
1つのnoteで両方を説明することはできない。**合わない明細は、何も書かない明細より悪い。**

BIRは課税なしの場合「bracket over 0: 0.00 + 0%」ではなく
「250,000の非課税枠以下なので源泉なし」と書く。

3水準で検証済み（MSC20k未満／超／課税対象）。**金額は一切変更していない。noteの文言のみ。**

⚠️ **既存の給与明細のnoteは変わらない。** 次回以降の給与計算から適用される。
**過去期間の再計算はしないこと（教訓37）** — 現在のスタッフプロファイルで上書きされる。

---

## ✅ 給与明細に計算根拠を出す（2026-09-01・本番反映済み）

### スタッフ質問「AED 7.2100/h の根拠は？」への回答

```
hourly = 基本給 ÷ 26日 ÷ 8時間
       = AED 1,500.00 ÷ 26 ÷ 8
       = 7.211538... → 7.21（2桁に丸め）
夜勤手当 = 4.11h × 7.21 × 10% = AED 2.96
```
定数は `app/dubai_payroll_engine.py` の `WORKING_DAYS_PER_MONTH=26` / `WORKING_HOURS_PER_DAY=8` / `NIGHT_RATE=0.10`。
該当者は **Abishek Rana Magar**（基本給1,500）。ドバイで1,500の人は13名。

⚠️ **`7.2100` という4桁表示は誤解を招いていた。** 実際は2桁に丸めた 7.21 を使っており、
スタッフが 1500÷26÷8=7.2115 を計算すると数字が合わない。**2桁表示に変更。**

### 修正内容

| 対象 | 変更 |
|---|---|
| ドバイ給与エンジン | note に**計算式と時給の導出**を追加。`_rate_basis()` に集約（夜勤・遅刻控除で共用） |
| My Pay（スタッフ明細） | マニラ行に **`quantity × unit_rate`** を表示。管理画面には元からあった |
| 同上 | note の `truncate` を廃止。**根拠を説明する後半が切り落とされていた** |

```
変更前  Night premium 4.11h @ AED 7.2100/h x 10% (22:00-04:00 GST)
変更後  Night premium: 4.11h x AED 7.21/h x 10% = AED 2.96
        (hourly = basic AED 1,500.00 / 26 days / 8h = AED 7.21; night window 22:00-04:00 GST)
```
...
| | スタッフ明細 | 管理画面 |
|---|---|---|
| マニラ | note ＋ **qty×rate（今回追加）** | `/admin/payroll/manila/[periodId]` が qty×rate＋note を表示 ✓（元から） |
| ドバイ | note（今回、式と導出を追加） | `/admin/payroll/adjustments` が note を省略なしで表示 ✓（元から） |

**「ページで計算式が見える」は元から概ね成立していた。** 不足していたのは
①ドバイ note に時給の導出が無い ②スタッフ側だけ qty×rate が見えない ③note が切り詰められる、の3点。

⚠️ **`quantity`/`unit_rate` が入っているのは3,958行中1,662行。**
SSS / PhilHealth / Pag-IBIG / MONTHLY_BASIC などは未設定で、note のみ。
社会保険料は料額表参照なので式が書きにくいが、**等級（bracket）を note に足すと検証しやすくなる**。未実施。
...
12:20（マニラ時間）に「CK 12:00 に遅刻」とアラートが鳴った。

**遅延ではない。** アラートは5分ごとに `shift_published_rows` を直接読んでおり、キャッシュは無い。
原因は**古い行が消えずに残っていたこと**。

```
TAFT 13:00-22:00 STAFF  updated 09-01 00:39  ← 新しい割当
CK   12:00-21:00 PIC    updated 08-28 05:52  ← 消えずに残存 → これで鳴った
```

### 欠陥1: publish が差分適用なので、付け替え前の行が残る（教訓31の副作用）

別支店に移すと新しいセルは書かれるが、**元の支店の行は消えない**。両方が有効なシフトとして返る。

修正: **時間が重なる、より新しい行がある場合のみ**古い行を落とす。
`DISTINCT ON (staff, date)` にすると**分割シフト**（同日 11:00-15:00 と 17:00-21:00）が消えるので不可。

### 欠陥2: 支店名を version（編集したシート）から取っていた

付け替えは**開いていたシートに書かれる**ので、version=CK / row=TAFT になる。
アラートは version 側を読んでいたため、勤務しない支店名を表示していた。
→ `r.branch_code` 優先（空なら version）。直近5,237行中1,587行が row 側 NULL。

### 欠陥3: 支店名の表記ゆれ

`shift_published_rows.branch_code` は同じ支店に2形式が混在:
`Business Bay` 655 / `BB` 144、`Al Mina` 600 / `AM` 5、`Al Barsha` 385 / `AB` 22、`Arjan` 279 / `ARJ` 56。
→ SQL側でコードに正規化。

---

### ⚠️ 未対応: この機能は実務で使われていない

```
60日で 509件のアラート（1日8〜16件）
  自動クローズ  497件（98%）
  人が対応      8件（UI 5 / Discord返信 3）
  「Auto-expired」= 出勤しないまま期限切れ  321件
DM受信者は13名
```

**鳴った時点では「20分遅れ」だが、ほとんどは本人が到着して自動で閉じる。**
受信者が見る頃には対応する対象が無い。一方で**321件は最後まで出勤していない**のに、
誰も動いた記録が無い。

改善案（未実施・要判断）:
1. **20分では早すぎる可能性** — 到着する人と来ない人が区別できていない。
   1回鳴らして終わりではなく、**未出勤のまま一定時間経過したものだけ再通知**する
2. **13名全員へのDMをやめる** — 誰が対応するか決まっていないので誰も動かない（教訓の「無言の行き止まり」）
3. **321件の未出勤を後から一覧で見られるようにする** — 現状はDiscordに流れて消えるだけ

---

## ✅ Prep Time: 正しいデータだけを採用（2026-09-01・本番反映済み）

### ⚠️ 最重要: Prep Time の過去データは大半が測定値ではない

元の写真を読み直して判明。**ドバイのgrabfoodラベルだけの問題ではない。**

| サンプル | レシートが写っていない | 保存分数が再読取りと一致 |
|---|---|---|
| Dubai grabfood 25件 | 12件 (48%) | 1/13 |
| **Manila grabfood 20件** | 14件 (70%) | 3/6 |
| **Dubai careem 20件**（ラベルは正しい） | 10件 (50%) | 2/10 |

全店舗が20.1〜22.2分に固まっているのは実力が拮抗しているのではなく**同じ値が生成されていた**ため。

**技術的原因の一つは解像度。** QC採点用に800pxへ圧縮した画像をレシート読取りにも使っていた。
同じ写真10枚で 800px→プラットフォーム判別0/10、**1568px→5/10成功**（careem/keeta/noon）。

### 採用基準（オーナー判断: 「正しいデータだけ採用します」）

**その都市に実在するプラットフォームの記録だけを集計する。** 判断ではなく事実で切れる基準。
個々の記録が正しく読めたかは数字からは分からないが、**GrabFoodはUAEに存在しない**ので
ドバイのgrabfood記録は読めていない証拠になる。
...
直近60日: **Dubai 5,886→1,576件（27%）／ Manila 2,998→2,276件（76%）**
※採用後の平均は 20.4〜22.0分でほぼ不変 — 全体の形は概ね正しかったが、個々の行は正しくなかった

### 除外の3分類（削除せず `excluded_reason` を付けて一覧に残す）

| 理由 | 内容 | 件数(60日) |
|---|---|---|
| `wrong_platform` | その都市に無いプラットフォーム | Dubai 3,165 / Manila 66 |
| `unread_photo` | プロンプトの例をそのまま返した（GF-192等） | 1,757 |
| `too_short` | `PREP_TIME_FLOOR_MIN`（既定3分）未満 | 45 |

**除外件数は画面とAPIに表示する。** 見えないフィルタは疑えない。

### 今後の記録に対する修正

- 都市ごとに実在するプラットフォームのみプロンプトに提示
- 「推測するな／読めなければ has_receipt:false」を明記
- 「Sushi Zen は自社の店名でプラットフォームではない」を明記（`zen`/`ZEN`/`zenith` 等を量産していた）
- サーバ側で例と一致する応答・都市に無いプラットフォームを拒否
- レシート読取りのみ 1568px（QC採点は800pxのまま）

### ⚠️ Taft が遅いという結論は撤回

同じ信頼できないデータでの比較だった。差は1〜2分で測定ばらつきの範囲。
**修正後のデータが2〜3週間貯まるまで、店舗間比較はしないこと。**

---

## ✅ Prep Time で Taft が選べない（2026-09-01・修正済み）

**Taft は記録を止めていない。** 同日に confirmed 10件あり、API も正しく返していた。3つが重なって隠れていた。

1. **既定期間が「月初〜今日」** → 毎月1日は**1日分だけ**の表示になる
2. **店舗ドロップダウンが表示中データから生成** → その期間に記録の無い店舗は**選択肢ごと消える**（＝店舗が無くなったように見える）
3. `toISOString()` は**UTC** → 東側の利用者は午前中「今日」が昨日になり、当日分が範囲外に落ちる

タイムスタンプで裏付け済み（2026-09-01 UTC）:
```
PAR   最初の記録 02:36  ← スクリーンショット時点(≈02:51 UTC)で存在 → 2件表示
TAFT  最初の記録 03:44  ← まだ無い → 選択肢からも消えていた
```

**修正**: 期間は月初〜今日だが**14日を下回らない** / 日付は**ローカル暦日** /
ドロップダウンは**過去90日に記録のある全店舗**を出し、当該期間に無いものは
`— no records in this range` と明記（消さずに、無いことを言う）。

⚠️ ドロップダウンの元は `lib/branches.ts` ではなく **stats API**。
`BRANCHES` は `CUB`、実データは `CUBAO` で不一致があり、混ぜると二重表示になる。
...
## ✅ OT「その夜は実際に忙しかったか」判定（2026-09-01・本番反映済み）

OTは「残業したから」申請が来るので却下できない。問題は**忙しい夜と平常の夜を見分ける手段が無かった**こと
（以前は夜間モニタリングで把握、現在は誰も見ていない）。各行に色と根拠を出す。

**2軸** — 同じ曜日・同じ店舗・過去9週の中央値と比較:

| 色 | 条件 |
|---|---|
| 🟢 ok | 注文が +15%以上 |
| 🟡 watch | ±15%以内（人員も通常以上なら「回避できたか確認」と明示） |
| 🔴 check | −15%以下 |
| ⬜ unknown | 履歴4日未満 → **色を出さない** |

**却下ボタンは追加していない。承認をブロックもしない。** 材料を出すだけ。

### ⚠️ 時間別POSデータを判定に使ってはいけない

`pos_sales_hourly_daily` はOTの時間帯と完全に一致するので理想的に見えるが、
**書き出しが欠ける日が多い**（ARJ/BB/JLT で29日中13日、AB は29日中23日が使用不能）。
欠けた日は「閑散だった」ように見える。

```
2026-08-22 Business Bay
  POS時間別 = 3件（21-23時）／通常19件 → −84% → 赤（誤り）
  dubai_order_counts = 114件 ← この期間の最繁忙日
```

**最も忙しい日に働いた人に「不要な残業」の赤を付けるところだった。**
判定は `dubai_order_counts`（120日中119日カバー）を使う。
時間別は日次と7割以上一致する日のみ `hour_detail` として**参考表示**、色は決めない。

### 人員数は `shift_published_versions` 経由で取る

`shift_published_rows.branch_code` は30日で787/3,708行が空欄（**OT申請者8名全員のシフトが該当**）、
かつ表記ゆれ（"Arjan"/"ARJ"、"Business Bay"/"BB"）。version側は全行に入っている。
**深夜跨ぎのOTは人員判定せず「不明」**（前日シフト扱いのため。0人と出すと不利な事実に見える）。

### 権限

`_OT_STAGE1_ROLES` に **DUBAI_MANAGEMENT が無く**、マニラのマネジメントはドバイのOTを承認できるのに
ドバイのマネジメントはできなかった（申請は全件ドバイ）。`channel.admin.overtime.manage` は**保有者0名の死に権限**だった。
`DEFAULT_ROLE_GRANTS` に追加（教訓33: DB直接投入は次回ログインで巻き戻る）。フロントの `STAGE1_ROLES` にも権限経路を `||` で追加（教訓25）。
→ **Rafael Jonas Lagahit が1回目Review可能に。Cyrine Fernandez は ADMIN なので元から可能**（ただし city=manila 登録）。

**現在の保留15件は 🟢12 / 🟡4 / 🔴0。** 8/27〜8/30 の Arjan は通常91〜104件のところ149〜152件で、
3名同時延長も数字と整合。**却下すべきものは無し。**

⚠️ **未確認: 実ブラウザでの表示確認**（ログインが必要）。バックエンドは実データで検証済み、tsc/eslint 通過。

---

## ✅ Paranaque Foodpanda 3日分を修正（2026-09-01）

検知が出した3件は**すべて実際の入力漏れ**だった。Foodpanda の注文明細CSVで確認:

| 店舗 | CSV実績 (8/10・8/11・8/12) | DB | 判定 |
|---|---|---|---|
| Cubao | 18 / 18 / 12 | 18 / 18 / 12 | 一致 |
| Taft | 35 / 30 / 34 | 35 / 30 / 34 | 一致 |
| **Paranaque** | **6 / 4 / 6** | 空欄 | **入力漏れ** |

**2店舗が1件も違わず一致し、警告を出した1店舗だけがずれていた。** 総数も併せて修正（81→87 / 57→61 / 52→58）。
検知は現在0件。

---

## ✅ マニラ日次売上：チャネル空欄の検知を追加（2026-09-01・本番反映済み）

7月の欠損は「その日の行が無い」で検知できたが、**8/31 は検知できなかった**。
3店舗とも行はあり、Foodpanda だけが空欄だったため。

**総数は4チャネルの合計として保存される**（8月93行すべてで一致）。
つまりチャネルを空欄にすると内訳が欠けるだけでなく**その日の総数そのものが減る**。
8/31 は Taft が約34件、Cubao が約15件少なく記録されていた。

`manila_daily_sales_gaps()` の検知を3種類に:

| 種別 | 内容 |
|---|---|
| `missing` | その日の行が1件も無い |
| `partial` | 店舗が足りない |
| `channel_blank` | **新規** 行はあるが、ほぼ毎日使うチャネルが空欄 |

**閾値 `regular_ratio=0.8` は実データから読んだ**（決め打ちではない）。
全 branch×channel が 0 / 2 / 49〜53%（Beep・Cubao店内）と 89〜100%（毎日使う）の
2群に分かれ、間が空いている。0.70〜0.85 は結果が同一。
**0.90 は Paranaque の Foodpanda 率に乗るため、窓の切り方で検知したりしなかったりする。**

**出す場所を Discord だけでなく画面にも**（教訓38）。
`GET /api/admin/analytics/manila/daily-sales/gaps` を追加し、
Sales Data Input タブ上部に該当日をボタンで表示。押すとその日付に飛び、保存すると消える。
検知が失敗しても入力は止めない（パネルが出ないだけ）。

**現在の未処理3件** — Paranaque 8/10・8/11・8/12 の Foodpanda 空欄。
同じ日に Cubao 18/18/12、Taft 35/30/34 と Foodpanda は稼働しており、
Paranaque の前後は 2〜6件。3日連続の空欄が実際に0だったのか入力漏れかは要確認。
※8/10・8/12 は `0`、8/11 は `NULL` で保存されている。

⚠️ **8/31 の3店舗も同じ状態だが、直近5日は猶予期間のため未報告**。9/5 に検知対象へ入る。

---

## 🔖 いま中断しているもの（2026-08-30）

| 件名 | 引き継ぎ | 次の一手 | 担当 |
|---|---|---|---|
| **マニラ・ドバイ VAT 実装** | [handoff/vat-implementation.md](handoff/vat-implementation.md) | アグリゲーター手数料の VAT 請求書をポータルで確認 | **Claude（未着手）** |
| ↑ マニラ TIN・法人構成 | 同上 #2 | `own_tax_numbers` にマニラ行が無い | 西村さん（確認中） |
| ↑ ドバイ QFZP 資格 | 同上 #3 | 法人税。VAT とは別問題 | 西村さん（確認中） |
| ドバイ `attendance_auto` 8/20〜 | 下記 | **未実行**。38名 / 149日 / AED 481.40 | 西村さん判断待ち |
...
**happy path の再実行では何も出ない。壊しにいって7件。**

| # | 内容 | 深刻度 |
|---|---|---|
| C1 | **`overdue_reviews()` を作ってUIから呼んでいなかった。** 報告は15件、画面は18件のまま | 高 |
| D1 | 試用期間の「延長」で**過去日を指定でき**、生成直後から期限超過の行ができる | 高 |
| G2 | `dismiss_nte_drafts_bulk` が**1件ごとにDB接続**。最大29件＝29往復。1文に統合し 29件0.01秒 | 高 |
| H1 | 1タップVerifyが**最初の検証者を上書き**。`COALESCE(NULLIF(...))` で保持、明示編集のみ上書き | 中 |
| F1 | `close_nte`/`record_nte_explanation` が**city未スコープ**。一覧はスコープ済みなのに書き込みが素通し | 中 |
| UX1 | 結果リストの取得失敗時、**ボタン0個＋保存不可のまま無説明**（Performance・NTE両方） | 中 |
| UX2 | Notice to Explain の「Active Notices」が**ただの数字**。押すと ACTIVE 絞り込みで History を開くように | 低 |

**確認して問題なしだったもの**
- `fp_total` は float で届く（文字列連結の心配なし）
- `employee_cases.manage` は旧Closeと同じ20名が保持 → **ロックアウトしていない**
- 空の一括処理・items 0件のオンボーディング・city='all' の文書 → いずれも該当なし/安全

**操作回数（実測）**
```
Performance 判断   カード「Decide」→結果→理由→保存   = 3タップ
NTE クローズ       Active Notices→Close→結果→保存    = 4タップ
現金ドラフト       理由選択→人ごと「Dismiss all」→確認 = 3タップ/人（22人）
Onboarding 検証    「Mark verified」                  = 1タップ/項目
```
⚠️ 現金ドラフトは22人×3タップ＝66タップ。**全件一括は意図的に作っていない**
（170件を無審査で消せてしまうため）。人単位が判断の正しい粒度。
...
**問題は入力不足ではなく、同じ事実を記録する「安い道」と「高い道」が併存し、安い道だけが何も残さない構造だった。**

| 実績（改修前） | 数字 |
|---|---|
| 応募者のうち求人票に未リンク | 115/132（87%） |
| `position_applied` の表記ゆれ | 132件に74通り |
| 面接以降のステータス vs 面接記録 / 評価 | 92名 vs 3件 / 5件 |
| 面接済みのまま7日以上未判断 | 55名（14日以上49名・最長49日） |
| 不採用のうち理由あり | 0/16 |
| 求人票 open のまま作成後未更新 | 19/19（うち目標日超過16件） |

**実装（4段階すべて本番・検証済み）**

| 段階 | 内容 | 検証 |
|---|---|---|
| 1 | `PositionPicker`（職種＋求人票を1項目に）／`create_applicants_bulk`（一括追加・重複スキップ） | 12/12 |
| 2 | `record_interview_outcome`（Proceed/Hold/Pass の3タップで評価＋理由＋ステータスを1トランザクション） | 25/25 |
| 3 | `hr_job_requisitions.openings` ／ `close_requisition_if_filled`（採用イベントで自動クローズ） | 28/28 |
| 4 | `hr_hiring_plans` ＋ `recruitment_overview`（Plans タブ・社長用画面） | 同上 |

**設計上の決めごと（変更するなら理由を確認すること）**
- 進捗は `hr_applicants` から算出する。`filled_count` 等は**保存しない**（更新忘れで壊れないため）
- 自動クローズは**採用イベント時のみ**。定期掃引にすると、人が意図的に再オープンした求人票を再び閉じてしまう
  - ⚠️ 再オープンしただけでは `filled >= openings` のままなので、次の採用で即座に再クローズする。増員なら `openings` を上げること
- Hold / Pass は理由必須。Proceed は不要（あとで聞かれるのはHoldとPassだから）
- Plans は新ページを作らず **Recruitment ページ内タブ**にした（新チャンネル登録・権限配布・Resync が不要）

**未着手**
- `created_by` の自動記録（応募者・求人票）— Offboarding は対応済み、Recruitment は未対応
- 既存19件の求人票は `openings=1` で移行済み。実際の必要人数は現場で入力が必要
- 既存の未リンク115名は遡って紐づけていない（今後の登録分から解消される）

**現場対応（システムと別に必要）**
- 面接済み55名の判断。最長は nepthali rodriguez（store manager / 47日待ち / 7-15以降）
- Cubao Manager 求人票は目標日 2026-07-10 を **52日超過**、候補9名・オファー0

## ✅ Store Supplier 発注 — 棚卸しの入力完了を待つ（2026-08-30）

現場報告：「TAFT で在庫が引かれず Par Level がそのまま Order に入る」。

**Par が数量に使われたのではない。`par − 0` が par と同じだっただけ。**

```
10:41 TAFT が PM レポートを開く
10:44 TAFT が AM レポートを開く    ← ここで「棚卸し済み」と判定される
11:01 ★ scheduler が発注 #156 を作成（186行中46行入力・保存0件）
12:00〜12:09  Bok choy / Cucumber / Mango … を入力
17:27 入力完了
```

ずれた13件は**すべて発注の1時間後に入力されたもの**。

### 原因

待ち合わせが **`latest_inventory_date()` — レポートが存在するか**しか見ていなかった。
**TAFTはAM/PMの2本を朝に開くので毎回この穴を踏む。**

### ⚠️ TAFT 固有ではない

```
TAFT 2026-08-30    68分早い
CUB  2026-08-20   276分早い
PAR  2026-08-20  2032分早い（前日の棚卸しで作成）
```

CUB / PAR は「今日たまたま問題なかった」だけ。

### 修正

**「その店のカタログ品目で、数え終わっていない品目が何件あるか」**で待つ。
棚卸し全186行ではなく**発注に必要な22件**だけを見る（全件待ちは17:22まで動けない）。

- 既定 **未入力1品目まで許容**。`SUPPLIER_ORDER_MAX_UNCOUNTED` で**デプロイ不要**に変更可
- **割合ではなく件数**。2品目未入力は CUB 89.5% / TAFT 90.9% となり、割合だと**同じ状況で店ごとに挙動が変わる**
- 22時の締切は維持（発注が出ないほうが困る）。保留理由と未入力の品目名を記録
- 明細行に `stock_counted` を保存。**「在庫0」と「数えていない」は後から区別できない**ため
- 発注詳細APIが `uncounted_items` を返す
...
| 全品目（棚卸しなし） | 4日 |
| 1品目 | 6日 |
| 2品目 | 1日（CUB 8/27、過剰発注は最大 ₱112） |
| 完全 | 75日 |

**保留は5日**（90%ルールと完全に同一）。8/30 の発注は 11:01 → 13:00頃に遅れる。

**検証**: 10/10 PASS。問題の3日はすべて保留、正常日は通過、リプレイで新規発注0件。

### 未了

`uncounted_items` は API までで**画面に未表示**。締切で強制発注された日に店舗へ見せるべき。
...
**検証**: 本番で 25/25 PASS。実ブラウザで警告と延長ボタンの表示まで確認。

### ⚠️ 追加のダブルチェックで5件のバグを発見・修正（同日）

happy path は通っていたが、**壊しにいったら5件出た**。

| 症状 | 実害 |
|---|---|
| **「Extend and save」が延長していなかった** | 統計上の日数から終了日を再計算して引数を捨てていた。ボタンが**警告の対象そのものの期間**を書いていた |
| **担当者を変更すると前の人の日が残る** | レコードを更新してから取り消していたので新しい名前で照合し0件マッチ。Louiela に105行が残ったまま Lynde に60行を追加 |
| **期間が重なる2件目が1件目の行を奪う** | `absences` の一意キーが (city, staff, date, type, source) なので同じ行を取り合う。1件目は60日中14日に減り、取り消すと2件目の日を消す |
| **ドバイの産休が何も止めない** | 日数はRA 11210のもの（UAEは45日）、ドバイエンジンはフラグを読まない。**記録されたように見えて控除は続く** |
| 統計日数を短縮する終了日が通る | 105日の権利を勝手に短くできた |

修正後、5件とも解消を確認（0 problems）。回帰スイート 25/25 も再実行して維持を確認。

⚠️ **バグBの副作用で本番に105行が残っていたので削除済み**（`absences` 105行・フラグ105件）。
...
退勤漏れ。通常は1日1件程度なので、この2日は**移行のコストであってスタッフの過失ではない**。

| | 件数 | 金額 |
|---|---:|---:|
| 7/09 missing_punch | 15 | AED 142.54 |
| 7/10 missing_punch | 5 | AED 49.82 |
| 7/10 break_excess（Sharon Namale） | 1 | AED 53.20 |
| **合計** | **21** | **AED 245.56** |

⚠️ **Sharon Namale の break_excess は退勤漏れの副産物**。`actual_break_minutes=489`
（所定60分）は「8時間休んだ」のではなく、休憩が閉じずに走り続けただけ。
7/10 の missing_punch 5件の合計より大きい。**同じ打刻漏れを二重に課金していた。**

**実装場所は engine（`app/dubai_payroll_engine.py`）**。手でDELETEするだけでは、
次に誰かが Generate を押した瞬間に全部戻る（生成側が毎回 delete → re-insert するため）。

- `PUNCH_GRACE_DUBAI="2026-07-09..2026-07-10"` — **デプロイ不要**で変更可、空文字で無効化
- 不正な値は「免除しない」側に倒す（黙って全免除にしない）
- **控除のみ。night premium 等の addition は必ず支払う** — 猶予が支給を減らしてはいけない
- `break_excess` は**同じ日に打刻漏れがある場合のみ**免除。窓内でも打刻が揃っていれば課金
- 免除した行は**氏名つきで返して画面に出す**（黙って消えると「計算漏れ」と区別できない）

既存21行は削除済み（バックアップ `_payroll_adj_rollout_grace_backup`、cycle 38 は open）。
7/9・7/10 の night premium 26件 AED 76.12 は残っている。

**検証**: 窓判定 13/13 PASS、使い捨てcycleで実際に Generate して 7/7 PASS
（waived 21 / missing_punch 0 / break_excess 0 / 書き込みは addition のみ）。

### ⚠️ 未決 — 西村さん判断待ち

- **7/10 の対象は5名**。依頼にあった4名に加えて **Ramuel M. Gatin**（AED 8.65）。
  日付が理由なので同じ扱いにした。**外すなら連絡が必要。**
- **6月にも missing_punch が15件ある**（OS勤怠データは 2026-06-01 から存在）。
  「7/9が練習」なら6月はさらに練習のはず。ただし依頼の範囲外なので**触っていない**。
  うち実際に控除が立っているのは 6/26・6/28 の2件（AED 19.23）だけ。
  含めるなら `PUNCH_GRACE_DUBAI="2026-06-01..2026-07-10"`。

---

## ✅ Management Channel — 回答を写真単位にした（2026-08-30）

現場から「C以下のアラートをまとめて送ると一つの問題として処理される」との報告。

**原因**: 製品スコアのアラートは Phase 0 で「店舗×日で1通」に集約したが、
**回答も1つのまま**だった。#877 は 13:30 / 13:40 / 14:49 の3枚を1組の選択肢で受けており、
**#859 は14枚を "other" の一言で閉じていた**。

**直し方**: タスクを写真ごとに分割し直すのではなく、**回答を分割した**。
分割するとTAFTは7日で91枚＝1日13通になり、Phase 0 で volume を絞った意味が消える。
- `context.answers[score_id]` に写真ごとの cause / action / channel / note を保存
- **全部の写真に回答するまで `responded` にしない**
- `response` / `response_action` はロールアップを維持（BO一覧とスコア集計が読むため）

**あわせて直した2件**:
- **Discord / Kitchen が表示されていなかった** — 選択は保存されていたが画面に出ていなかった。
  「feedback given」で止まっており、BOが読みに行けない。店舗の回答カードと BO Dashboard の
  両方に per-photo でバッジ表示。西村さんの方針（できる限りDiscord）が測定可能になった。
- **写真が全digestタスクで 404 だった** — `source_id` が `CUB:2026-08-30` になったのに
  エンドポイントは int 変換のままだった。エラー時は非表示なので誰も気づけない。
  `context` から id を取り、`?item=` で日内の何枚目かを指定できるようにした。

**検証**: バックエンド 17/17（Heroku one-off）。本番ブラウザで #877（実データ・3枚）の
表示を確認し、使い捨てタスクで回答→ `1/2 done` → 完了 → BO の per-photo 表示まで往復。

---

## ✅ 2026-08-30 に直したもの（すべて本番反映済み）

現場から上がった報告を起点に9件。**共通する型は「静かに失敗していた」** —
成功と表示されるのに書かれていない、権限があるのに効かない、送信済みなのに誰にも届いていない。

| # | 症状（報告者の言葉） | 原因 | 対処 |
|---|---|---|---|
| 1 | **My Pay が表示されない**（マニラ数名） | ①`verify_access_token` が city 無しで権限解決し、`scope_type='city'` の付与が**全員分**落ちていた（CK_MANILA が 42→3）②在籍75名に `staff_role_assignments` が無く `staff_master.role`＝STAFF にフォールバック | 権限キャッシュのキーに city/branch を追加／75名へ追加割当／`staff_role_assignments` に version bump トリガ／新規登録時も割当を作る |
| 2 | **Disposal 未提出のアラートが誤報** | 走査がマニラ 00:00。締め作業は日付をまたぐ（直近14日中11日） | 02:00 まで猶予（`MGMT_DISPOSAL_GRACE_HOUR`）／提出済みタスクの自動クローズ／7日で期限切れ／解決済みへの Send は 409 |
| 3 | **Send to Francis と出るのに送信エラー** | ボタンは当番表を実時間で読み、ガードは保存済み `manager_name` を読んでいた。デプロイ直後の5件が空だったのは `resolve_task_owner` が読み取り経路で DDL を走らせロック待ちしたため | 読み取り経路から DDL を除去／ガードも当番表で解決してから拒否 |
| 4 | **Replied by に未回答者の名前** | `manager_name` は宛先であって回答者ではない | 未送信／送信済み未回答／回答済みを区別して表示 |
| 5 | **複数アラートが1オーダーに集約** | `product_score_c` の要約 `source_id` が店舗×日で共通 | 既存要約は `open` のときだけ延長。旧225件の `count`/`summary` を後埋めし、送信済み6通の本文も修復 |
| 6 | **Payslip の合計が60ペソ合わない** | 明細から都度計算した内訳と、保存済み run の金額がずれる（Adjustment は再計算しない） | 一致するときだけ内訳を表示。ずれていれば警告バナー |
| 7 | **Payslip の単位が全部 day(s)** | 11コードのうち7つは時間・分・割合 | `ITEM_QTY_UNIT` でコード別に表示 |
| 8 | **Publish 98件と出るが5セルしか直していない** | 旧ドラフト移行時、`branch_code` を含む完全一致で比べていたため同一98件中84件が持ち越し | `cellDraftSignature`（start/end/role のみ）で比較。検証: 84→0 |
| 9 | **Absences ページが 500** | `list_absences` が11列に増えたのに呼び出し側が8列で展開 | dict 返しに変更し名前で参照 |

### #9 の停止期間（現場から再報告があったため実測）

| | リリース | 時刻 (+04) |
|---|---|---|
| 壊れた | v2376 `c7070762` | **2026-08-28 19:55** |
| 直った | v2403 `522ee165` | **2026-08-30 11:24** |

**約39.5時間**、Dubai / Manila とも Absence Report も History も表示されなかった。
該当期間に絞り込み日付内に欠勤が1件でもあれば 500、0件なら 200 で空表示になるため、
現場からは一様に「何も出ない」に見えた。**2026-08-30 に本番で再検証済み**
（Report 17件・History 69件・Staff絞り込み 2件。絞り込みは別SQLなので個別に確認）。

### 併せて入れたもの

- **OT → Payroll**（①〜⑥）: 承認と加算を分離／Add to Payroll・Remove from Payroll／
  同期を「上書き」から「照合」に変更し残骸を自動修復／未加算 OT の警告
  → マニュアル [overtime-manual.html](../manuals/overtime-manual.html)
- **VAT Phase 0 の検算**: `GET /api/admin/vat/source-uniqueness`（毎月の申告前に実行）
...
**宛先も通知も存在しない。** 322件中 `manager_name` が入っているのは1件（"Test Manager"）。
BO が送った10件に返信は0件。

**対象はマニラのみ**（TAFT / PAR / CUB / CK）。ドバイは営業4年目で安定しているため今回は外す
（西村さん判断 2026-08-29）。ただしドバイでもタスクは生成され続けるので（7日で72件）、
**Send のガードを全都市に効かせないこと** — 効かせるとドバイが恒久的に送信不可になる。
...
### ⚠️ Phase 0 を配送より先に置く理由

宛先を付けるだけでは直らない。**マニラは1日31.4件生成され、うち21.3件が
`product_score_c`（QC写真1枚につき1行）で、回答率は 2/11 ＝ 18%。** TAFT だけで1日12.4件。
このまま配送を通すと担当1人に1日約15通届き、その4分の3は受け取った側に次の動作が無い。
**1週間で通知を切られる。そのとき記録上は「送信済み」になり、Compliance Score の分母に入り、
届いていないのに本人の点が下がる。** 今日クリアした90件と同じ構図が通知付きで再生産される。

答えられている型とそうでない型は既に分かれている（`rush_check_missing` 3/3＝100%、
`backup_below_50/70` 2/2、`disposal_missing` 2/4 に対し `product_score_c` 2/11）。
違いは**受け取った人の次の動作が一意に決まるかどうか**。
...
**当番表5名分の Discord ユーザーIDは全て揃った**（2026-08-29）。対応表は仕様書に記載。

⚠️ **当番表の TAFT 月曜と Francis 休務時の代理は `Ayako Nishimura`**（2026-08-29 変更）。
原文の「私」＝西村さんだったが「開発と経営で店舗管理はできない」ため差し替え。
**`Yukihiro Nishimura` は当番表に載せない。** これに伴いエスカレーションは
Yusuke ⇄ Ayako の相互になる（2人が最上位。**3段目を足さないこと**）。
...
⚠️ **クリアは対症療法。同日中に11件再発した**（全て 8/29 10:02〜10:48 UTC の46分間）。
BO は今も押している。Phase 1 が入るまで1日約11件のペースで溜まり続ける。

**未確定は無し**（2026-08-29 に3件とも決着）:
- `ALL` の宛先 → **`Yusuke Uejima`**（全曜日）。実在2件は全社KPI（prime cost / food cost）で
  店舗では実行できず、`bo_assignee` も既に彼だった
- 担当が休みのとき → **公開シフトで自動判定しない**。当番表に `代理` 列（TAFT=Ayako）を持たせ、
  BO が Send を押す瞬間に「本日シフト外」と代理への切替を出す（教訓18: 自動判定は誤っていたとき
  沈黙して別人に届く）。最終の網は24時間エスカレーション
- `Francis Ibana` の Discord ID → 受領済み

**BO への確認1件（実装は止まらない）**: `complaint_no_photo` / `pm_backup_missing` /
`salmon_yield_alert` を送っていない理由。**採点からは先に外してよい**ので回答待ちは不要。
...
実務は Peter Villafuerte 個人に依存している。**その依存を外すのが依頼の目的。**
...
打刻から復元できるか実測した。**入社日と打刻の両方がある55名で、完全一致は14名（25%）。**
入社日が空の23名は「初出勤日」が取れるが全員 `2026-04-01` ＝ **OS が勤怠を取り始めた日**で、
入社日ではない（Peter が2026年4月入社でないことは明らか）。

**役職・入社日は契約書・201ファイルにしか存在しない。** `staff_master.role` はアクセス権限であって職名ではない。

### ⚠️ 既存レコードの編集を必須ゲートにしないこと
...
## ✅ Completed: Phase 1 税務項目の抽出 (2026-08-26)

### 実施
`invoice_ocr_service` のプロンプトを税務用に拡張し、`drive_invoices` 208件を再抽出。

```
tin_or_trn / tin_present / receipt_type /
amount_excl_tax / tax_amount / tax_rate_pct
```

列は**追加のみ**。既存列と値は一切触っていないので、現在このテーブルを読んでいる処理の挙動は不変。
...
| 処理済み | 208 / 208件（未処理0） |
| TRN印字あり | 180件 |
| なし・社内振替 | 23件 ← **判別1件も誤りなし** |
| なし・外部 | 5件（実体は FOODICS 1件 + 自社店舗 ZEN 4件） |
| 税抜/税額の分離 | 180件 |
| **検算不一致** | **0件**（税抜+税額=税込、VAT 5%） |

### 🔴 重要な発見：TRNの「値」は使えない
| 取引先 | 件数 | 異なるTRN | 最頻値の占有率 |
|---|---:|---:|---:|
| Chef Middle East | 54 | **40通り** | 22% |
| SAWHNEY FOODSTUFF | 26 | 17通り | 15% |
| OCEAN FISHERIES | 22 | 15通り | 32% |
| SUNBERRY | 29 | 2通り | 90% |
| SUMMIT TRADING | 14 | 3通り | 86% |

多数決でも上位3社が収束しない。桁数不正も8件（12/14/16桁、UAEは15桁が正）。
**長い数字列はこの種のモデルが最も苦手とする対象。**

一方 **「TRNが印字されているか」は完全に信頼できる**（社内振替23件を無誤判別）。

→ **設計変更**：抽出値は signal として保存し、真の値は `vendor_master` に
**25社ぶん人手で1回登録**する。抽出は「有無の検出」と「マスタとの不一致検出」に使う。
これは元の方針「中核はAIではなく取引先マスタ」の実測による裏付けでもある。

### 副次的な修正
- `max_tokens` 2000 → 4000（明細が多い1件でJSONが途中で切れていた。再試行で成功）
- 通貨別の桁数検証を追加（AED=15桁）
...
### きっかけと、現場からの本質的な指摘
Taft の断水が夕方に発生し、HQに届いたのは夜。現場の指摘:
> 断水は「インシデント」だと思わなかった。インシデントレポートは
> 「自社の不手際があり、対応策を考えるための材料」だと認識していた。

**実データが指摘を裏付けた:**
```
自社起因 15件 ／ 外部起因 0件
Weather / Facility / Equipment → 最初から選択肢にあるのに、一度も使われていない
```
→ **障壁はカテゴリ不足ではなく「インシデント」という言葉そのもの**だった。
Waterカテゴリを足すだけの前案では、そもそも画面を開かない人に届かなかった。

### 設計：認識を問わない
「認識を変えてもらう」＝170名への教育施策は、マネージャーの判断力に依存するのと
同じ脆さ。代わりに**認識を問わない画面**にした。

**緊急度と原因は独立した軸**。報告者に判断できるのは「いま困っているか」だけ。

`/store/report` — 扉は2つ、部屋は1つ（同じテーブル・同じHQ受信箱）:
| 扉 | 行き先 |
|---|---|
| いま起きている — 助けが要る | 速報（6項目・即通知・応答まで再通知） |
| 起きたこと — 記録しておきたい | 従来の Incident Report |

### カテゴリ→レベルの自動判定（重大度は選ばせない）
```
Water / Electricity / Gas / Security          → Level 2（無条件）
Fire / Injury / Flood / Forced Closure        → Level 3
Equipment / Weather / Facility / Other        → 営業影響ありなら 2、なければ 1
```
実測: `Water,影響なし→2` / `Equipment,影響なし→1` / `Equipment,影響あり→2` ✅

### エスカレーション（旧実装の欠陥を修正）
🔴 旧: `critical>2h` `high>4h` のみ、しかも**一度通知したら二度と対象外**
→ これが16件中8件が `new` のまま滞留した直接の原因。

新: **誰かの名前が付くまで鳴り続ける**。Level3=10分毎 / Level2=30分毎 / Level1=24時間毎。
投稿時に即時通知（毎時の掃引待ちにしない）。ワーカーの安全網は毎分チェック
（ループは10秒周期なので明示的に絞った）。

### 引き取りには名前が必須
`acknowledge_incident` は名前なしを拒否、二重引き取りも拒否（先着を返す）。
現場に責任を求めながらHQ側に空白を残さないため。

### 発見→報告の遅延を記録
`reported_lag_min` を投稿時に計算・保存。**未来の発見時刻は400で拒否**
（既存16件中2件が報告時刻より後という不正データだったため）。

### 本番実測
```
未来時刻     → 400 "The time it was noticed cannot be in the future." ✅
断水(2h前)   → 200 level=2 reported_lag_min=120 ✅
即時通知     → notify_round=1 ✅
名前なし引取 → 拒否 ✅ / 二重引取 → 先着を返す ✅
引取後       → 通知対象から外れる ✅
```

### 残（ユーザー判断待ち）
- Level 3の電話連絡先と順番（システム外）
- HQ側の一次受け運用（現状は権限を持つ全員が引き取り可能）
- 管理画面に「未引き取り一覧」と速さ指標（`/api/admin/incidents/speed` は実装済）

⚠️ **指標に通報件数を使わないこと。** 少ないほど良く見え、報告しない動機を作る。
測るのは「発見→報告の速さ」と「HQの引き取りの速さ」だけ。
...
- 仕入VAT回収不可: 従業員以外への接待 / 私用可能な社用車 / 法令契約外の従業員向け支出
- 法人税50%のみ損金: 接待交際費（Decree-Law 47 第32条）
- 全額損金不算入: 罰金・過料、賄賂、非適格寄付、法人税自体
- 控除の前提: 仕入先TRN入りの有効なTax Invoice ← **Vendorsページの存在理由**

接待判定は科目ではなく**1枚ごと**に決まるため、ドバイは科目リストではなく伝票フラグで持つ。

⚠️ 教訓: 「外部の確定待ち」と書く前に、その情報が公開の様式に印刷されていないか確認する。
実際に必要だったのは科目リストではなく「過去の申告でどの行を使ってきたか」（整合性のため）で、
着手を止める理由にはならなかった。
...
## ✅ Completed: 「支出合計」の二重計上を修正 + 勘定科目マスタ (2026-08-26)

### 発見
`mgmt_overhead` に **`支出合計` という費目**があり、個別費目と**並べて合算**されていた。
合計行が科目として混入していた。

| 月 | 修正前 | うち支出合計 | 修正後（正） | 過大 |
|---|---:|---:|---:|---:|
| 2025-10 | 3,442,990 | 1,019,296 | 2,423,694 | +42% |
| 2025-11 | 4,108,583 | 1,409,907 | 2,698,677 | +52% |
| 2025-12 | 3,048,470 | 1,785,192 | 1,263,279 | +141% |
| 2026-01 | 3,228,452 | 2,327,881 | 900,571 | +258% |
| 2026-02 | 3,951,236 | 2,894,356 | **1,056,880** | **+274%** |

**これが「Manila overhead 2025-10..2026-02 が直近月の約3倍」の正体。** 期間が完全一致。
個別費目が連続系列である証拠：2026-02 は27種で1,056,882、2026-03 は26種で1,181,443、
2026-07 は27種で1,280,808。3月に落ちたのは費用減ではなく**二重計上が止まったから**。

### 勘定科目マスタ（59種 → 22科目）
- `account_master` 22科目 + `cost_center_master` 5拠点 + `overhead_category_map` 59行
- **`mgmt_overhead` は書き換えていない。** 生の費目はそのまま残し、マップを通して読む。
  → Q1〜Q5 の判断が後から変わってもマップ1行の UPDATE で済み、履歴も失われない
- 家賃8種（家賃/家賃Rent/CK家賃/CK家賃Rent/KD家賃/Warehouse家賃/スタッフルーム家賃/その他家賃など）
  → `RENT` 1科目。Manila 2026-07 は 615,079 として1行で出る（464,329+75,000+42,750+33,000）
- 資産計上（キッチン機器等初期投資 3,114,447 PHP）を `CAPEX_EQUIP` として経費から分離
- 原価センターは **store_code を置き換えず3軸目として追加**
  （store_code は「どの店が負担するか」＝配賦先。CK家賃は25,000×3店に配賦済み）
- 未マッピング 0 件

### 修正箇所
`get_mgmt_cost_summary` 3か所（合計 / 繰越プローブ / 費目別）と
`get_mgmt_daily_pl` 2か所。日次PLも同じ二重計上を日割りしていた。

### 検証
2025-11〜2026-02 の4か月で月次 = 日次×日数が完全一致。
2025-10 のみ日次が0だが、これは**売上データが 2025-11-16 開始**のため（既存挙動・本件と無関係）。
Dubai は変化なし（12〜13科目）。

### 未回答（マップの1行修正で反映可能）
- Q1: overhead の「食材費」1,084,589 PHP（2025-10〜12のみ）は二重計上か
- Q2: 「KD」とは何か（cost_center type が UNKNOWN のまま）
- Q3: 配送3種（CKデリバリー/Lalamove/Dispatch Fee）を分けるか
- Q4: 「フィリピン外注費」257,591 AED はグループ間取引か
- Q5: 「名義貸し」50,000 PHP の科目区分

### 重要な整理
**この作業は管理会計（月次PL）用であって、税務申告の仕訳ではない。**
経理担当のレシート仕訳作業は1件も減らない。税務側には別途、
会計事務所の勘定科目体系が必要（`account_master.tax_account_code` を用意済み）。
...
### 確定前の2段階警告（現場からの指摘を受けて追加）
「間違って押したらマネージャーに解除を依頼」と案内しようとしたが、実データを確認すると
**Manila CK / WH / BO の25名は全員 STAFF ロールで、解除できる人は0名**だった。
解除可能なのは **HQ の6名のみ**（全員 dubai 登録）。マニラにマネージャーロール保有者はいない。
→ 実行できない案内だったため、Finalize を2段階に変更:

- **Stage 1**（未入力がある場合のみ）: 件数を大きく表示し
  「終わっていないなら戻って Save を押す」「自分では解除できない」を明示。
  戻るボタンを紫の大ボタン、進むボタンを小さい枠線に。
- **Stage 2**: 最終確認。「Only the office can reopen it」

全件入力済みなら Stage 1 はスキップ。

当初は英語＋タガログ語の併記で実装したが、翻訳の質が不十分との判断で**英語のみ**に変更。
中途半端な翻訳は、理解される必要がある唯一の画面では無い方がよい。
将来入れる場合はマニラのメンバーに原文を書いてもらうこと。
