...
### エスカレーション先 → **Yusuke Uejima**、ただし本人の担当分は **Ayako Nishimura**
24時間無反応のとき。
...
Yusuke は CUB / CK の火木土日の担当でもあるため、素直に実装すると
**彼が落としたタスクが彼自身に上がる。** 2026-08-29 に西村さんが Ayako Nishimura を指定。

⚠️ **一般則として実装すること。** 「エスカレーション先が担当者本人と一致したら、次の宛先へ」。
名前を2つ書き分けるだけの実装にすると、担当表が変わった瞬間に同じ輪が戻る。
`Ayako Nishimura` が将来どこかの担当になったら、彼女の分の逃げ先も要る。
...
3. ⚠️ **Yusuke Uejima・Yukihiro Nishimura・Ayako Nishimura は
   `staff_master.city='dubai'` で登録されている。**
   タスクは `city='manila'`。受信箱は city で絞るため、**そのままでは自分宛が見えない可能性が高い。**
   TAFT月曜の担当（西村さん）、CUB/CK の火木土日（Yusuke）、**エスカレーション先（Ayako）**が
   まとめて該当する。**当番表を正しく作っても、この3人の分だけ届かない**という結果になりうる。
