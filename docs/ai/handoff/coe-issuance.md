...
### Phase A — 項目を持てるようにする <span>実施済 2026-08-29</span>

- ✅ `staff_master.company`（`SUSHIZEN` / `7CZ`）と `staff_master.position` を追加。
  **`position` は給与プロファイルから名簿へ移した** —— 理由は下記のバグ
- ✅ Staff 作成フォームに役職・入社日・法人を追加。**`city='manila'` のときのみ必須**。
  入社日は 2025-08-01 より前を拒否
- ✅ 一括入力画面 `/admin/staff/employment-details`。名簿軸・店舗別・BOで開く。
  Staff の導線カードから到達
- ✅ `set_employment_details` が入社日を名簿と給与プロファイルの両方へ、1箇所から書く

⚠️ **実装中に自分で作ったバグ（記録）**

役職の置き場を当初 `manila_staff_profiles` のみにしていた。ところが
**名簿90件のうち16件は給与プロファイルを持たない**（`Test Account` もその1つ）。
UPDATE が0件で終わり、APIは `ok` を返し、画面は「保存しました」と表示した。
Camilla の作業リストの **18%で入力が消える**ところだった。教訓15と同型。

対処：`position` を名簿へ移し、**名簿への書き込みは必ず rowcount を検査**して
該当が無ければ名前を挙げて拒否する。給与側への書き込みは意図的に best-effort
（プロファイルが無いのは16名にとって正常な状態であり、失敗ではない）。

**現状（本番実測）:** 未入力 89件 / 役職なし 57 → 名簿へ33件を引き継ぎ済 /
入社日なし 36 / 法人なし 89（新設列のため全員）
