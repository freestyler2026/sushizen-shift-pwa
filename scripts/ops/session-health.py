...
判定に使う手掛かりは2種類あり、確度が違う。混ぜずに分けて出す。
  ...
  0. ⚠️ 2026-09-11 追記: probe は **CIが使う .b64 ファイル**を読む。以前は
     ローカルの .json を読んでいて、talabat は json が通り b64 が失効という
     ずれが実際に起きていた（取込が死んでいるのに🟢）。あわせて、トークンが
     発行できても取込が叩くAPIが401なら「半死」と出す。
...
#   foodpanda … 入金 + 注文台帳 + 店舗状態（開いているかを日中3回読む）
# したがってセッションが切れると止まるのは入金だけではない。ワークフロー全体の
# 赤/緑で見ているので新しいステップも自動的に入るが、**止まる範囲はこれ**。
WORKFLOWS = {
