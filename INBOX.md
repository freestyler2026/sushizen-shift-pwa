## [2026-09-09] 発注金額を後から変えても承認レベルが再計算されない
理由: 承認ロジックに触る。Session 2 の軽微修正の範囲外。

`replace_direct_purchase_items`（db.py）は `total_amount` を再計算するが、
`current_approval_level` と `required_roles_json` は**作成時のまま**。
BO の修正画面は明細を丸ごと差し替えられるので、₱15,000 の発注を ₱150,000 に
書き換えても MANAGER 承認のまま通る。

**実測（2026-09-09、DRAFT と未ルーティングを除外した実数）: 11件。**

| 発注 | 金額 | 承認された階層 | 金額が要求する階層 |
|---|---:|---|---|
| MAN-PR-202607-0380 | ₱92,086.64 | lvl1 MANAGER | **lvl3 HQ** |
| MAN-PR-202608-0103 | ₱25,325.00 | lvl1 MANAGER | lvl2 HR_MANAGER |
| （他9件、₱20,160〜₱24,384） | | lvl1 | lvl2 |

10件は ₱20,000 の境界のすぐ上なので、作成時の金額が境界未満だった可能性が高い。
**₱92,086 の1件だけは桁が違う。**

直し方は「保存時に金額から階層を引き直す」。マトリクスは
`proc_approval_matrix_php` にあるので、作成時と同じ関数を通せばよい。
⚠️ ただし**既存の承認済みを遡って作り直さないこと**（教訓37）。

**あわせて: 承認マトリクス自体が dyno 再起動のたびに既定値へ戻る。**
`ensure_procurement_default_matrix()` はプロセス内フラグでしか守られておらず、
`ON CONFLICT (level_no) DO UPDATE` で無条件に上書きする。つまり
`/admin` の画面で閾値を変えても、次のデプロイで消える（教訓20・33と同型）。
現在4行とも `updated_by='system_seed'` なので、誰の設定も生き残っていない。
...
**確定した仕様（2026-09-09 オーナー確認済み。会計事務所への質問リストからは外す）**
- 上限: `gratuity = MIN(計算値, basic_salary × 24)`。**基本給ベース**であって総支給ではない。
  到達は勤続約27年からで現状該当者はいないが、実装はしておく。
- 基本給の定義: 住宅手当・交通費・コミッション・残業代は**すべて除外**。
  MOHRE登録契約書の基本給のみ。
- 勤続1年未満は対象外。1年超なら端数は日割り。
- 自己都合退職でも減額しない（現行法では退職も解雇も同じ算式）。
- 支払期限は契約終了から**14日**（Article 53）。2024年改正で罰則強化。
  → 期限までの残日数を画面に出すこと。

**⚠️ 無給期間の扱い — ここが実装の落とし穴**
```
勤続日数 = 総日数 − 無給休暇日数（ただし産休・傷病休暇の無給部分は差し引かない）
```
産休105日のうち45日全額・15日半額・以降無給、傷病は15日全額・30日半額・45日無給。
**この無給部分を勤続から引くと過少計算になり、MOHRE申立ての対象になる。**

**⚠️ JLT は DIFC 管轄（他4拠点はメインランド）**
JLT の **ACTIVE 10名（ドバイ62名中）** に連邦法の21/30日算式を当ててはいけない。
DIFC は独自規則（DEWS）。**都市ではなく拠点で分岐する**必要がある。

**実データの状況（2026-09-09 実測）**
- `absences.absence_type` は種別を持っている:
  `DAY_OFF / VACATION_LEAVE / ABSENT / MATERNITY_LEAVE / INJURY / MEDICAL_LEAVE`
  ドバイに実際の **MATERNITY_LEAVE 30日・1名**（2025-11）あり。産休の判別は可能。
- **ただし「無給」を表す種別が存在しない。** `reason_category` も
  `SICK / PERSONAL / OTHER / FAMILY` で有給無給を区別しない。
  無給日を特定する手段は公開シフトの `UL` コードのみ（ドバイ10行・1名）。
- つまり**種別は absences、無給かどうかはシフト、と記録が2つに割れている。**
  上の算式を実装するには、無給フラグを absences 側に持たせる必要がある。

**設計メモ**
- 計算根拠（勤続日数・除外した無給日数・適用レート・端数按分・上限適用の有無）は
  **画面に出す**（教訓9）。数字だけ出すと監査で使えない。
- マニラ項目とドバイ項目は**同じ列を使い回さない**。`fp_separation_pay` に EOSG を
  入れると、両都市の集計が別物を足すことになる。
- 会計事務所に聞くのは別の3点: ①Gratuity引当の会計処理（現在PLに計上が無い）
  ②DIFC/ADGM該当の確認 ③2024年改正の罰則内容。
