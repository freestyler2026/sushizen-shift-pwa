...
function normalizeApiErrorMessage(raw: string, fallback: string) {
  ...
  if (text.includes("<!DOCTYPE html") || lower.includes("<html") || lower.includes("application error")) {
    return "Server timed out while loading menu data. Please retry.";
  }
...
async function parseText(res: Response, fallback: string): Promise<string> {
  const text = await res.text();
  if (!res.ok) {
    throw new Error(normalizeApiErrorMessage(text, fallback));
  }
  return text;
}

export async function menuGet<T = any>(path: string): Promise<T> {
  const res = await requestWithAuth(path, { method: "GET" });
  return parseJson<T>(res, `GET ${path} failed`);
}

export async function menuPost<T = any>(path: string, body: Record<string, unknown>): Promise<T> {
...
export async function menuPatch<T = any>(path: string, body: Record<string, unknown>): Promise<T> {
  const res = await requestWithAuth(path, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
...
export async function menuGetText(path: string): Promise<string> {
  const res = await requestWithAuth(path, { method: "GET" });
  return parseText(res, `GET ${path} failed`);
}
