...
export default function StoreProcurementReceivingPage() {
  ...
  const [closeNotReceivedManagerName, setCloseNotReceivedManagerName] = useState("");
  const [closeNotReceivedManagerPin, setCloseNotReceivedManagerPin] = useState("");
  ...
  const closeOrderNotReceived = useCallback(async () => {
    ...
    try {
      await procurementJson(
        ...
        {
          method: "POST",
          body: JSON.stringify({ approver_name: mgrName, pin: mgrPin, reason: closeNotReceivedReason.trim() }),
        },
        requestedBy,
        pin,
      );
  ...
  }, [requestId, requestedBy, pin, closeNotReceivedReason, closeNotReceivedManagerName, closeNotReceivedManagerPin, loadMyRequests, loadReceivings]);
          ...
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-zinc-900 p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-amber-800/30 bg-amber-950/10 p-3 space-y-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCloseNotReceivedManagerPin(e.target.value)}
                    placeholder="••••••"
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-zinc-600 focus:border-amber-600/50 focus:outline-none"
                  />
                </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => { setCloseNotReceivedOpen(false); setCloseNotReceivedManagerName(""); setCloseNotReceivedManagerPin(""); }}
