...
export default function MenuTagsPage() {
  ...
  if (!ready) return <div className="text-sm text-neutral-500">Loading tags...</div>;
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-[380px,1fr]">
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/20 p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-neutral-100">{editingId ? "Edit Tag" : "Create Tag"}</div>
              <div className="mt-1 text-xs text-neutral-400">Tags support bulk updates and CSV import/export for large-scale operations.</div>
            </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block text-sm text-neutral-300">
                ...
                <input value={form.color} onChange={(e) => setForm((current) => ({ ...current, color: e.target.value }))} className="w-full rounded-xl border border-neutral-700 bg-neutral-950/50 px-3 py-2 text-sm" />
              </label>
              <label className="block text-sm text-neutral-300">
          ...
          <div className="mt-4 flex gap-2">
            <button type="button" onClick={() => void saveTag()} disabled={saving} className="rounded-xl border border-amber-700 bg-amber-950/30 px-4 py-2 text-sm text-amber-100 disabled:opacity-50">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/20 p-5">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading ? <tr><td className="py-4 text-neutral-500" colSpan={7}>Loading tags...</td></tr> : rows.length ? rows.map((row) => (
                  <tr key={row.id} className="border-t border-neutral-800/80 align-top">
                    ...
                    <td className="py-3 pr-4">
                      ...
                      <div className="mt-1 text-xs text-neutral-500">{row.name_localized || row.reference || "-"}</div>
                    </td>
                    <td className="py-3 pr-4">
                      <div className="inline-flex items-center gap-2 rounded-full border border-neutral-700 px-2 py-1 text-xs text-neutral-200">
                        <span className="inline-block h-3 w-3 rounded-full" style={{ backgroundColor: row.color || "#A16207" }} />
                        {row.color || "#A16207"}
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-4 text-neutral-300">{Number(row.usage_count || 0)}</td>
                    ...
                    <td className="py-3">
                      <div className="flex flex-wrap gap-2">
                        <button type="button" onClick={() => { setEditingId(row.id); setForm({ name: row.name || "", name_localized: row.name_localized || "", reference: row.reference || "", color: row.color || "#A16207", sort_order: String(row.sort_order ?? 0) }); }} className="rounded-xl border border-neutral-700 px-3 py-1.5 text-xs text-neutral-200">Edit</button>
                        ...
                        {row.status !== "DELETED" ? <button type="button" onClick={() => void deleteTag(row.id)} className="rounded-xl border border-rose-800/80 px-3 py-1.5 text-xs text-rose-200">Delete</button> : null}
                      </div>
                    </td>
                  </tr>
                )) : <tr><td className="py-4 text-neutral-500" colSpan={7}>No tags found.</td></tr>}
              </tbody>
            </table>
          </div>
