...
export default function MenuModifierGroupsPage() {
  ...
  const loadRows = useCallback(async (nextCity = city, nextTab = tab, nextQ = q, nextPage = page, nextPageSize = pageSize) => {
    ...
    try {
      const res = await menuGet<PaginatedResponse<ModifierGroupRow>>(
        `/api/admin/menu/modifier-groups?city=${encodeURIComponent(nextCity)}&tab=${encodeURIComponent(nextTab)}&q=${encodeURIComponent(nextQ)}&page=${encodeURIComponent(String(nextPage))}&page_size=${encodeURIComponent(String(nextPageSize))}&sort_by=sort_order&sort_dir=ASC`,
      );
      setRows(res.rows || []);
  ...
  async function saveGroup() {
    if (!form.name.trim()) {
      setError("Please enter modifier group name.");
      return;
    }
    setSaving(true);
    setError("");
    ...
    try {
      ...
      if (editingId) {
        await menuPatch(`/api/admin/menu/modifier-groups/${encodeURIComponent(editingId)}?city=${encodeURIComponent(city)}`, payload);
        setSuccess("Modifier group updated.");
      } else {
        await menuPost("/api/admin/menu/modifier-groups", { city, ...payload });
        setSuccess("Modifier group created.");
      }
      resetForm();
  ...
  async function updateStatus(groupId: string, status: "ACTIVE" | "INACTIVE") {
    ...
    setSuccess("");
    try {
      await menuPost(`/api/admin/menu/modifier-groups/${encodeURIComponent(groupId)}/status`, { city, status });
      setSuccess(`Modifier group marked ${status.toLowerCase()}.`);
      await loadRows(city, tab, q, page, pageSize);
    } catch (e: any) {
      setError(e?.message || String(e));
    }
  ...
  async function deleteGroup(groupId: string) {
    ...
    setSuccess("");
    try {
      await menuPost(`/api/admin/menu/modifier-groups/${encodeURIComponent(groupId)}/delete?city=${encodeURIComponent(city)}`, {});
  ...
  async function exportRows() {
    setWorking(true);
    try {
      const text = await menuGetText(`/api/admin/menu/modifier-groups/export?city=${encodeURIComponent(city)}&tab=${encodeURIComponent(tab)}&q=${encodeURIComponent(q)}&sort_by=sort_order&sort_dir=ASC`);
      const blob = new Blob([text], { type: "text/csv;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `menu-modifier-groups-${city}.csv`;
      link.click();
  ...
  async function importRows(file: File) {
    ...
    setImportFailures([]);
    try {
      const res = await menuPost<{ success_count: number; failed_count: number; failures?: ImportFailure[] }>(`/api/admin/menu/modifier-groups/import?city=${encodeURIComponent(city)}`, { csv_text: await file.text() });
  ...
  async function applyBulkAction() {
    ...
    try {
      ...
      const res = await menuPost<{ success_count: number; failed_count: number }>(`/api/admin/menu/admin-tools/modifier-groups/bulk?city=${encodeURIComponent(city)}`, { action: bulkAction, ids: selectedIds, values });
  ...
  if (!ready) {
    return <div className="text-sm text-neutral-500">Loading modifier groups...</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl border border-neutral-800 bg-neutral-900/20 p-5">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[140px,minmax(220px,1fr),auto]">
            ...
            {["ALL", "ACTIVE", "INACTIVE", "DELETED"].map((value) => (
              <button
                ...
                className={[
                  ...
                  tab === value ? "border-amber-500 bg-amber-950/25 text-amber-200" : "border-neutral-700 bg-neutral-950/40 text-neutral-300",
                ].join(" ")}
              >
                {value}
              </button>
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-[380px,1fr]">
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/20 p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-neutral-100">{editingId ? "Edit Modifier Group" : "Create Modifier Group"}</div>
              <div className="mt-1 text-xs text-neutral-400">Groups define the option set that can later be linked to products.</div>
            </div>
            {editingId ? (
              <button type="button" onClick={resetForm} className="rounded-xl border border-neutral-700 px-3 py-2 text-xs text-neutral-300">
                New
              </button>
            ) : null}
          </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <label className="block text-sm text-neutral-300">
              ...
              <input value={form.sort_order} onChange={(e) => setForm((current) => ({ ...current, sort_order: e.target.value }))} className="w-full rounded-xl border border-neutral-700 bg-neutral-950/50 px-3 py-2 text-sm" />
            </label>
            <label className="block text-sm text-neutral-300">
              <div className="mb-1 text-xs text-neutral-500">Description</div>
              <textarea value={form.description} onChange={(e) => setForm((current) => ({ ...current, description: e.target.value }))} className="min-h-28 w-full rounded-xl border border-neutral-700 bg-neutral-950/50 px-3 py-2 text-sm" />
            </label>
          </div>
          ...
          <div className="mt-4 flex gap-2">
            <button type="button" onClick={() => void saveGroup()} disabled={saving} className="rounded-xl border border-amber-700 bg-amber-950/30 px-4 py-2 text-sm text-amber-100 disabled:opacity-50">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/20 p-5">
          <div className="flex items-center justify-between gap-3">
            <div className="text-sm font-semibold text-neutral-100">Modifier Groups</div>
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td className="py-4 text-neutral-500" colSpan={8}>Loading modifier groups...</td></tr>
                  ...
                  <tr key={row.id} className="border-t border-neutral-800/80 align-top">
                    ...
                    <td className="py-3 pr-4">
                      ...
                      {row.name_localized ? <div className="mt-1 text-xs text-neutral-500">{row.name_localized}</div> : null}
                    </td>
                    <td className="py-3 pr-4 text-neutral-300">{row.reference || "-"}</td>
                    ...
                    <td className="py-3 pr-4 text-neutral-300">{Number(row.linked_product_count || 0)}</td>
                    ...
                    <td className="py-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {row.status === "ACTIVE" ? (
                          <button type="button" onClick={() => void updateStatus(row.id, "INACTIVE")} className="rounded-xl border border-neutral-700 px-3 py-1.5 text-xs text-neutral-200">
                            Inactivate
                          </button>
                        ) : row.status === "INACTIVE" ? (
                          <button type="button" onClick={() => void updateStatus(row.id, "ACTIVE")} className="rounded-xl border border-emerald-700/80 px-3 py-1.5 text-xs text-emerald-200">
                            Activate
                          </button>
                        ) : null}
                        {row.status !== "DELETED" ? (
                          <button type="button" onClick={() => void deleteGroup(row.id)} className="rounded-xl border border-rose-800/80 px-3 py-1.5 text-xs text-rose-200">
                            Delete
                          </button>
                        ) : null}
                      </div>
                  ...
                  <tr><td className="py-4 text-neutral-500" colSpan={8}>No modifier groups found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
