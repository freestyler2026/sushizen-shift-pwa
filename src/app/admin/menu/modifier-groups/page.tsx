...
export default function MenuModifierGroupsPage() {
  ...
  async function saveGroup() {
    if (!form.name.trim()) {
      setFormError("Please enter modifier group name.");
      return;
    }
    setSaving(true);
    setFormError("");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-white">Modifier Groups</h1>
          <p className="mt-0.5 text-sm text-zinc-500">
            Manage modifier groups for{" "}
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr,auto]">
          <div>
            ...
            <SelectDark
              ...
              className={SELECT}
              options={[
                { value: "", label: "All modifier groups" },
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[380px,1fr]">
          ...
          <div className="mb-5 flex items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-white">{editingId ? "Edit Modifier Group" : "New Modifier Group"}</h2>
              </div>
              <p className="mt-0.5 text-xs text-zinc-500">
                {editingId ? "Editing existing group" : "Fill in the fields below"}
              </p>
            </div>
            {editingId ? (
              <button
        ...
        <div className={`${CARD} p-5`}>
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold text-white">Modifier Groups</h2>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-white/8">
                  ...
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Name</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Reference</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Sort</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Options</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Products</th>
              ...
              <tbody>
                {loading ? (
                  <tr><td className="py-6 text-sm text-zinc-500" colSpan={8}>Loading modifier groups...</td></tr>
                  ...
                  >
                    ...
                    <td className="py-3 pr-4">
                      ...
                      {row.name_localized ? <div className="mt-0.5 text-xs text-zinc-500">{row.name_localized}</div> : null}
                    </td>
                    <td className="py-3 pr-4">
                      {row.reference ? (
                        <span className="rounded-md border border-white/10 bg-white/6 px-1.5 py-0.5 font-mono text-xs text-zinc-300">{row.reference}</span>
                      ) : (
                    ...
                    <td className="py-3 pr-4">
                      <span className="inline-flex items-center rounded-full border border-sky-500/30 bg-sky-500/10 px-2 py-0.5 text-xs font-semibold text-sky-400">
                        {Number(row.option_count || 0)}
                      </span>
                    </td>
                    <td className="py-3 pr-4">
                      <span className="inline-flex items-center rounded-full border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-xs font-semibold text-violet-400">
                        {Number(row.linked_product_count || 0)}
                      </span>
                    </td>
                  ...
                  <tr><td className="py-6 text-sm text-zinc-500" colSpan={8}>No modifier groups found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
