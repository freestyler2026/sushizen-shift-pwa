...
function MenuCombosPageInner() {
  ...
  async function saveCombo() {
    if (!form.name.trim()) return setFormError("Please enter combo name.");
    setSaving(true);
    setFormError("");
    setError("");
    setSuccess("");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-white">Combos</h1>
          <p className="mt-0.5 text-sm text-zinc-500">
            Manage combo items for{" "}
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[380px,1fr]">
          ...
          <div className="mb-5 flex items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-white">{editingId ? "Edit Combo" : "New Combo"}</h2>
              </div>
              <p className="mt-0.5 text-xs text-zinc-500">
                {editingId ? "Editing existing combo" : "Fill in the fields below"}
              </p>
            </div>
            {editingId ? (
              <button
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={form.price} onChange={(e) => setForm((current) => ({ ...current, price: e.target.value }))} className={INPUT} />
                  </div>
                  <div>
        ...
        <div className={`${CARD} p-5`}>
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold text-white">Combos</h2>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-white/8">
                  ...
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Combo</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Sort</th>
                  <th className="pb-2.5 pr-4 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Price</th>
              ...
              <tbody>
                {loading ? (
                  <tr><td className="py-6 text-sm text-zinc-500" colSpan={7}>Loading combos...</td></tr>
                  ...
                  >
                    ...
                    <td className="py-3 pr-4">
                      <Link href={`/admin/menu/combos/${encodeURIComponent(row.id)}${searchParams.toString() ? `?${searchParams.toString()}` : ""}`} className="font-medium text-violet-300 hover:text-violet-200">
                        {row.name}
                      </Link>
                    ...
                    <td className="py-3 pr-4">
                      <div className="text-zinc-200">{Number(row.price || 0).toFixed(2)}</div>
                      <div className="mt-0.5 text-xs text-zinc-500">Cost {Number(row.cost_summary?.effective_cost || 0).toFixed(2)}</div>
                    </td>
                    <td className="py-3 pr-4 text-zinc-300">{Number(row.product_count || 0)}</td>
                    <td className="py-3 pr-4"><span className={statusBadge(row.status)}>{row.status}</span></td>
                    <td className="py-3">
                      <div className="flex flex-wrap gap-1.5">
                        <button
                          type="button"
                          onClick={() => { setEditingId(row.id); setForm({ name: row.name || "", name_localized: row.name_localized || "", sku: row.sku || "", barcode: row.barcode || "", image_url: row.image_url || "", description: row.description || "", price: String(row.price ?? 0), pricing_method: row.pricing_method || "FIXED_PRICE", costing_method: row.costing_method || "FROM_INGREDIENTS", fixed_cost: String(row.fixed_cost ?? 0), sort_order: String(row.sort_order ?? 0) }); }}
                        ...
                        {row.status !== "DELETED" ? (
                          <button
                            type="button"
                            onClick={() => void deleteCombo(row.id)}
                ...
                )) : <tr><td className="py-6 text-sm text-zinc-500" colSpan={7}>No combos found.</td></tr>}
              </tbody>
            </table>
          </div>
...
export default function MenuCombosPage() {
  return (
    <Suspense fallback={<div className="flex items-center gap-3 text-sm text-zinc-500"><span className="h-4 w-4 animate-spin rounded-full border-2 border-violet-500 border-t-transparent" />Loading combos...</div>}>
      <MenuCombosPageInner />
    </Suspense>
  );
}
