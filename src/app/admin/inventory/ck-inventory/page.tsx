...
export default function CkInventoryPage() {
  ...
  useEffect(() => {
    if (!ready || !allowed) return;
    if (tab === "count" && masterItems.length === 0) void loadMaster(city);
    if (tab === "history") void loadHistory(city);
  }, [ready, allowed, tab, city, masterItems.length, loadMaster, loadHistory]);
          ...
          <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-neutral-800 bg-neutral-900/30 p-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-xs text-neutral-500">
                {masterItems.length} items · {Object.keys(countDraft).filter((k) => parseFloat(countDraft[k] || "0") !== 0).length} entered
              </span>
              <button
                type="button"
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-neutral-800 bg-neutral-900/60 text-xs uppercase tracking-wide text-neutral-500">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-right">Counted Qty</th>
                </tr>
              </thead>
              <tbody>
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-neutral-800 bg-neutral-900/60 text-xs uppercase tracking-wide text-neutral-500">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-right">Item Count</th>
                </tr>
              </thead>
              <tbody>
                {historyRows.map((row, i) => (
                  <tr
                    key={`${String(row.count_date)}-${i}`}
                    className="border-t border-neutral-800 text-neutral-200 transition hover:bg-neutral-900/30"
                  >
