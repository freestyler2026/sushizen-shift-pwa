...
export default function CoePage() {
  ...
  const createRequest = async () => {
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) throw new Error(d?.detail || `HTTP ${res.status}`);
      setMsg({ kind: "ok", text: `Request logged for ${staffName}.` });
      setStaffName("");
      setSnap(null);
      setPurpose("");
      await load();
  ...
  const issue = async (r: CoeRow) => {
    if (!sigName.trim()) {
      setMsg({ kind: "err", text: "Enter the signatory's name first." });
      return;
    }
    const verb = r.issue_count > 0 ? "Re-issue" : "Issue";
    if (
      !window.confirm(
        `${verb} the Certificate of Employment for ${r.staff_name}?\n\n` +
          `Signed by: ${sigName}\n` +
          `This is a formal document handed to the employee. Check the details before you go ahead.`
      )
    )
      return;
    setBusy(r.id);
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) throw new Error(d?.detail || `HTTP ${res.status}`);
      setMsg({ kind: "ok", text: `${r.staff_name} — ${verb.toLowerCase()}d (issue no. ${d.issue_no}).` });
      await load();
    } catch (e: any) {
      setMsg({ kind: "err", text: e?.message || String(e) });
    } finally {
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className={T_CAPTION}>
            DOLE Labor Advisory 06-20 — must be issued within <b>3 days</b> of the request.
          </p>
        </div>
      </div>
      ...
      {overdue > 0 ? (
        <div className="flex items-start gap-2 rounded-2xl border border-rose-900/50 bg-rose-950/20 px-4 py-3 text-sm text-rose-200">
          ...
          <span>
            <b>{overdue}</b> past the 3-day deadline. A late COE is grounds for a DOLE complaint.
          </span>
        </div>
      ) : null}
      ...
      <div className={`${GLASS_CARD} space-y-4 p-5`}>
        <p className={T_SECTION}>New request</p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <input
              ...
              className={INPUT_CLASS}
            />
            <p className="mt-1 text-[11px] text-neutral-500">The 3-day deadline counts from this date.</p>
          </div>
          <div>
            <div className={T_LABEL + " mb-1.5"}>Purpose (optional)</div>
            <input
              value={purpose}
              onChange={(e) => setPurpose(e.target.value)}
              placeholder="e.g. for a new employer"
              className={INPUT_CLASS}
            />
          </div>
        </div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                ["Hire date", snap.hire_date],
                [snap.is_current ? "Employment" : "Last working day",
                  snap.is_current ? "Currently employed" : snap.last_working_date],
              ].map(([k, v]) => (
                <div key={String(k)}>
                  <div className={T_CAPTION}>{k}</div>
                  <div className="mt-0.5 text-sm text-neutral-100">
                    {v || <span className="text-rose-300">Not on record</span>}
                  </div>
                </div>
              ))}
            </div>
            ...
            {snap.can_issue ? (
              ...
              >
                {busy === "create" ? "Logging…" : "Log this request"}
              </button>
            ) : (
              ...
              <div className="mt-4 rounded-lg border border-amber-500/30 bg-amber-950/15 p-3">
                <p className="text-sm font-semibold text-amber-200">This COE cannot be issued yet</p>
                ...
                <p className="mt-2 text-[12px] text-amber-100/70">
                  Check the contract and record it under Staff → Employment Details.
                  Anything guessed here is printed on the certificate as fact.
                </p>
              </div>
            )}
          </div>
        ) : snap && !snap.found ? (
          <p className="text-sm text-rose-300">No one by that name on the roster.</p>
        ) : null}
      </div>
      ...
      {canApprove ? (
        <div className={`${GLASS_CARD} space-y-3 p-5`}>
          <p className={T_SECTION}>Signatory</p>
          <p className={T_CAPTION}>
            Chosen per issue. The name typed here is the one printed on the certificate.
          </p>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <div className={T_LABEL + " mb-1.5"}>Signatory name</div>
              <input
                value={sigName}
            ...
            <div>
              <div className={T_LABEL + " mb-1.5"}>Position</div>
              <input
                value={sigPosition}
      ...
      {/* ── Open requests ── */}
      <div className={`${GLASS_CARD} space-y-3 p-5`}>
        <p className={T_SECTION}>Waiting to be issued ({open.length})</p>
        {rows === null ? (
          <p className={`${T_CAPTION} py-6 text-center`}>Loading…</p>
        ) : open.length === 0 ? (
          <p className={`${T_CAPTION} py-6 text-center`}>Nothing waiting.</p>
        ) : (
          open.map((r) => (
            <div
              key={r.id}
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className={T_CAPTION}>
                    Asked {r.request_date} · due {r.due_date} ·{" "}
                    <span className={r.overdue ? "font-semibold text-rose-300" : ""}>
                      {r.overdue ? `${r.days_waiting} days waiting — overdue` : `${r.days_waiting} days waiting`}
                    </span>
                    {r.purpose ? ` · ${r.purpose}` : ""}
                  </p>
                </div>
                {canApprove ? (
                  ...
                  >
                    {busy === r.id ? "Issuing…" : "Approve and issue"}
                  </button>
                ) : (
                  <span className={T_CAPTION}>Waiting for an approver</span>
                )}
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 border-t border-white/5 pt-3 sm:grid-cols-4">
                <div>
                  <div className={T_CAPTION}>Company</div>
                ...
                <div>
                  <div className={T_CAPTION}>Position</div>
                  <div className="text-sm text-neutral-200">
                    {r.position}
                    {srcTag(r, "position")}
                  </div>
                ...
                <div>
                  <div className={T_CAPTION}>Hire date</div>
                  <div className="text-sm text-neutral-200">
                    {r.hire_date}
                    {srcTag(r, "hire_date")}
                  </div>
                ...
                <div>
                  <div className={T_CAPTION}>Last working day</div>
                  <div className="text-sm text-neutral-200">
                    {r.is_current ? "— (still employed)" : r.last_working_date}
      ...
      {done.length > 0 ? (
        <div className={`${GLASS_CARD} space-y-2 p-5`}>
          <p className={T_SECTION}>Issued and rejected ({done.length})</p>
          {done.map((r) => (
            <div
              key={r.id}
            ...
            >
              <div>
                ...
                <span className={`${T_CAPTION} ml-2`}>
                  {r.status === "ISSUED"
                    ? `Issued by ${r.approved_by}${r.issue_count > 1 ? ` (${r.issue_count} times)` : ""}`
                    : `Rejected: ${r.reject_note || "—"}`}
                </span>
              </div>
              {r.status === "ISSUED" ? (
