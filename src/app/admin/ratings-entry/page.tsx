...
/**
 * Standalone Dubai aggregator ratings entry (no import from @/components/admin/RatingEntryTab).
 * Survives stale /admin bundles; uses same API + auth as the tab version.
 */
...
type RatingBrand = keyof typeof RATING_GRID_CONFIG;
const BRAND_ORDER = ["Sushi Zen", "Ramen Zen", "All Veggie Sushi", "J-Deli"] as RatingBrand[];

type RatingCell = { score: number | null; count: string };
type RatingGridData = Record<string, RatingCell>;

function cellKey(brand: string, agg: string, branch: string) {
  return `${brand}|${agg}|${branch}`;
}

function parseRatingCell(raw: string): RatingCell {
  const s = raw.trim();
...
function formatRatingCell(cell: RatingCell): string {
...
const BRAND_DOT: Record<RatingBrand, string> = {
  "Sushi Zen": "#6366f1",
  "Ramen Zen": "#f97316",
...
const BRAND_BORDER: Record<RatingBrand, string> = {
...
const BRAND_TITLE: Record<RatingBrand, string> = {
...
function BrandGrid({
  ...
  for (const agg of aggs) {
    for (const branch of branches) {
      if (gridData[cellKey(brand, agg, branch)]?.score != null) filled += 1;
    }
  }

  return (
    <div className={`overflow-hidden rounded-xl border ${BRAND_BORDER[brand]}`}>
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <h2 className={`flex items-center gap-2 text-sm font-bold ${BRAND_TITLE[brand]}`}>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/10">
              ...
              {branches.map((branch) => (
                <th key={branch} className="min-w-[90px] px-2 py-2 text-center text-[10px] font-medium uppercase tracking-wide text-gray-500">
                    ...
                    <td key={branch} className={`px-1 py-1 ${scoreBgClass(cell.score)}`}>
                      <input
                        ...
                        onFocus={() => {
                          ...
                          setTimeout(() => {
                            const el = document.getElementById(`standalone-rcell-${k}`) as HTMLInputElement | null;
                            el?.select();
                          }, 0);
                        }}
                        onChange={(e) => {
                          setEditingText(e.target.value);
                          onChange(brand, agg, branch, e.target.value);
                        }}
                        onBlur={() => {
      ...
      <div className="flex items-center justify-between border-t border-white/10 px-4 py-2">
        <span className="text-[10px] text-gray-600">
          {selectedDate} · {aggs.length} × {branches.length}
        </span>
...
export default function RatingsEntryStandalonePage() {
  ...
  const loadDate = useCallback(
    async (date: string) => {
      ...
      setSaveError("");
      try {
        const qs = new URLSearchParams({ record_date: date, approver_name: nm, pin: p });
        const json = await apiGet<{ rows?: Array<{ brand: string; aggregator: string; branch: string; rating_score: number | null; review_count?: string }> }>(
  ...
  const handleCellChange = (brand: RatingBrand, agg: string, branch: string, rawText: string) => {
    const k = cellKey(brand, agg, branch);
    setGridData((prev) => ({ ...prev, [k]: parseRatingCell(rawText) }));
  ...
  const saveBrand = async (brand: RatingBrand) => {
    ...
    const rows = cfg.aggregators.flatMap((agg) =>
      cfg.branches.map((branch) => {
        const cell = gridData[cellKey(brand, agg, branch)] ?? { score: null, count: "" };
        return { aggregator: agg, branch, rating_score: cell.score, review_count: cell.count };
      }),
    );
    try {
  ...
  const handleKeyDown = (
    ...
    const nextK = cellKey(brand, cfg.aggregators[nextAgg], cfg.branches[nextBranch]);
    const el = document.getElementById(`standalone-rcell-${nextK}`);
    if (el) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className={T_PAGE_TITLE}>⭐ Ratings Entry</h1>
          <p className="mt-1 text-xs text-neutral-500">Standalone page · Dubai · same API as Admin → Ratings tab</p>
        </div>
      ...
      <div className={`${GLASS_CARD} space-y-3 p-4`}>
        <p className={T_CAPTION}>Approver name and PIN (HQ) — required to load and save.</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={() => shiftDate(-1)}
          className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-lg text-gray-300 hover:bg-white/10"
        >
          ‹
        </button>
        <input
          ...
          className="cursor-pointer rounded-lg border border-white/15 bg-white/5 px-4 py-2 font-mono text-sm text-white focus:border-indigo-500 focus:outline-none"
        />
        <button
          type="button"
          onClick={() => shiftDate(1)}
          disabled={selectedDate >= todayStr}
          className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-lg text-gray-300 hover:bg-white/10 disabled:opacity-30"
        >
          ›
        </button>
        {selectedDate !== todayStr ? (
          <button
            ...
            className="rounded-lg bg-indigo-500/10 px-3 py-1.5 text-xs text-indigo-400 hover:bg-indigo-500/20"
          >
            Today
          </button>
        ) : null}
        {loading ? (
          <span className="flex items-center gap-2 text-xs text-gray-500">
            <span className="h-3 w-3 animate-spin rounded-full border border-indigo-500 border-t-transparent" />
            Loading...
          </span>
        ) : null}
        <div className="ml-auto flex flex-wrap gap-3 text-[10px] text-gray-500">
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-sm border border-emerald-500/30 bg-emerald-500/20" /> ≥ 4.5
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-sm border border-amber-500/30 bg-amber-500/20" /> ≥ 4.0
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-sm border border-red-500/30 bg-red-500/20" /> &lt; 4.0
          </span>
        </div>
      </div>
      ...
      <p className="text-center text-xs text-neutral-600">
        Bookmark: <span className="font-mono text-neutral-400">/admin/ratings-entry</span>
      </p>
    </div>
  );
}
