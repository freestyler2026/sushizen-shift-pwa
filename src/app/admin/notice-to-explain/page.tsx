    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-white/10 bg-zinc-900/95 shadow-2xl backdrop-blur-sm">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {staffNtes.length === 0 && (
            <p className={T_BODY}>No NTEs on record.</p>
          )}
          {staffNtes.map((nte) => (
...
function TemplateModal({
  ...
  const handleSave = async () => {
    ...
    try {
      const h = authHeaders();
      if (isEdit && template) {
        const res = await fetch(`${apiBase}/api/admin/conduct/templates/${template.id}`, {
          method: "PATCH",
        ...
        const res = await fetch(`${apiBase}/api/admin/conduct/templates`, {
          method: "POST",
          headers: { ...h, "Content-Type": "application/json" },
...
export default function NtePage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      const raw = getAuth();
      if (!raw?.accessToken) {
        router.replace("/login?next=/admin/nte");
        return;
      }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Main dashboard data
      const res = await fetch(`${API_BASE}/api/admin/conduct/data`, {
        method: "POST",
        headers: h,
      ...
      // Board ranking
      const boardRes = await fetch(`${API_BASE}/api/admin/conduct/board`, {
        method: "POST",
        headers: h,
        body: JSON.stringify({ city }),
      });
  ...
  const handleIssueNte = async () => {
    ...
    setSuccessMsg("");
    try {
      const res = await fetch(`${API_BASE}/api/admin/conduct/issue`, {
        method: "POST",
      ...
      const msg = (resData as any).suspension_triggered
        ? `NTE issued to ${issueStaffName}. Suspension auto-created!`
        : `NTE issued to ${issueStaffName}. Active NTEs: ${(resData as any).active_nte_count ?? "—"}`;
      setSuccessMsg(msg);
      setIssueStaffName("");
      ...
      setError(e?.message || "Failed to issue NTE");
    } finally {
      setIssuing(false);
    }
  };
  ...
  const handleResolveNte = async (nteId: string, staffName: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/admin/conduct/${nteId}/resolve`,
        {
          method: "POST",
          headers: { ...authHeaders(), "Content-Type": "application/json" },
      ...
      setError(e?.message || "Failed to resolve NTE");
    }
  };
  ...
  const handleDeleteTemplate = async (id: string, title: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/admin/conduct/templates/${id}`,
        {
          method: "DELETE",
          headers: authHeaders(),
        }
  ...
  // How many active NTEs does issueStaffName currently have?
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className={`${T_BODY} mt-1`}>
            Issue Notice to Explain, track violations, manage enforcement.
            3 active NTEs trigger an automatic enforcement action.
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className={KPI_CARD}>
          <p className={KPI_LABEL}>Active NTEs</p>
          <p className={`${KPI_VALUE} ${totalActive > 0 ? "text-red-400" : ""}`}>
            {loading ? "—" : totalActive}
          </p>
        </div>
        <div className={KPI_CARD}>
          <p className={KPI_LABEL}>Total NTEs</p>
        ...
        <div className="space-y-3">
          ...
          {ranking
            ...
            .map((staff) => {
              ...
              // Build dot indicators for each NTE: active = filled, resolved = outline
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <span className={`text-lg font-bold ${color}`}>
                        {emoji} [{staff.total_count}{staff.total_count === 1 ? " NTE" : " NTEs"}]
                      </span>
          ...
          {willTriggerSuspension && issueStaffName.trim() && (
            <div className="flex items-start gap-2 rounded-xl border border-red-500/40 bg-red-950/20 px-4 py-3">
              ...
              <p className="text-sm text-red-300">
                ...
                has {issueStaffActiveCount} active NTE{issueStaffActiveCount !== 1 ? "s" : ""}.
      ...
      {tab === "history" && (
        <div className="space-y-4">
          ...
          <div className={`${GLASS_CARD} overflow-x-auto`}>
            {loading ? (
              <p className={`${T_BODY} p-5`}>Loading history…</p>
            ) : filteredHistory.length === 0 ? (
              <p className={`${T_BODY} p-5 text-center`}>No records found.</p>
            ) : (
        ...
        <div className="space-y-4">
          ...
          {!loading && templates.length === 0 && (
            <div className={`${GLASS_CARD} p-8 text-center`}>
              ...
              <p className={T_BODY}>No templates yet. Create one to speed up NTE issuance.</p>
            </div>
          )}
