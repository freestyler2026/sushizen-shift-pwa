    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className={`mt-0.5 ${T_CAPTION}`}>インシデントレポート管理</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className={KPI_CARD}>
          ...
          <p className={KPI_VALUE}>{counts.total}</p>
        </div>
        <div className={KPI_CARD}>
          <p className={KPI_LABEL}>新規</p>
          <p className={`${KPI_VALUE} text-rose-400`}>{counts.new}</p>
        </div>
        <div className={KPI_CARD}>
          <p className={KPI_LABEL}>対応中</p>
          <p className={`${KPI_VALUE} text-violet-400`}>{counts.in_progress}</p>
        </div>
        <div className={KPI_CARD}>
          <p className={KPI_LABEL}>解決済</p>
          <p className={`${KPI_VALUE} text-emerald-400`}>{counts.resolved}</p>
        </div>
      </div>
      ...
      <div className={`${GLASS_CARD} flex flex-wrap items-center gap-3 p-4`}>
        ...
        <div className="flex items-center gap-2">
          <label className={T_LABEL}>ステータス</label>
          <select
            className={`${SELECT_CLASS} w-auto min-w-[120px]`}
            value={filterStatus}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/8">
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>重要度</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>カテゴリ</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>店舗</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>投稿者</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>日時</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>ステータス</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-left`}>返信</th>
                  <th className={`${TABLE_HEADER} px-4 py-3`}></th>
                </tr>
              </thead>
              <tbody>
                {items.map((item) => (
