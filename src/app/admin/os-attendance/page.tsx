    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => void loadRecipients()} className="rounded-lg border border-white/10 bg-white/5 p-1.5 text-zinc-400 hover:text-white transition-colors" title="Refresh">
              <RefreshCw size={13} />
            </button>
        ...
        {showAddForm && (
          <div className="mb-4 rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="grid grid-cols-3 gap-3 mb-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 mb-1">City (optional)</label>
                <SelectDark value={newCity} onChange={v => setNewCity(v as "" | "dubai" | "manila")}
                  options={[{ value: "", label: "All cities" }, { value: "dubai", label: "Dubai only" }, { value: "manila", label: "Manila only" }]} />
              </div>
            </div>
            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead>
                <tr className="border-b border-white/10 text-xs text-zinc-500 uppercase tracking-wider">
                  ...
                  <th className="pb-2 pr-4 text-left font-semibold">City</th>
              ...
              <tbody>
                {recipients.map(r => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/3">
                    ...
                    <td className="py-2.5 pr-4 text-zinc-300">{r.city ?? "All"}</td>
                    ...
                    <td className="py-2.5 text-right">
                      {r.is_active && (
                        <button onClick={() => void handleRemove(r.id)} className="text-zinc-600 hover:text-red-400 transition-colors p-1" title="Remove">
                          <Trash2 size={13} />
                        </button>
                      )}
                    </td>
...
type Tab = "report" | "staff_report" | "gps" | "corrections" | "compliance" | "late_alerts" | "request_alerts";
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setTab("request_alerts")} className={`${tab === "request_alerts" ? TAB_ACTIVE : TAB_INACTIVE} flex items-center gap-1.5`}>
            📋 Request Alerts
          </button>
        </div>
        ...
        <div className={GLASS_CARD + " p-6"}>
          ...
          {tab === "request_alerts" && <RequestAlertsTab />}
        </div>
      </div>
    </main>
  );
