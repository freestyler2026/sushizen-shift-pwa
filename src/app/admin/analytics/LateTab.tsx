...
import { ChevronDown, ChevronRight, AlertTriangle, Users, Building2, Search, AlarmClock } from "lucide-react";
...
export default function LateTab({
  ...
  // Summary stats from current branch data
    ...
    <div className="space-y-4">
      ...
      {!loading && filteredBranch.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <KpiCard label="Late Incidents" value={totalIncidents} sub="qualifying days" />
          <KpiCard label="Staff Affected" value={totalStaff} />
          <KpiCard label="Total Late" value={fmtMins(totalLateMin)} sub="all staff combined" />
        </div>
      )}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/8">
                  ...
                  <th className={`${TABLE_HEADER} px-4 py-3 text-right`}>Total Late</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-right`}>Avg Late</th>
                  <th className={`${TABLE_HEADER} px-4 py-3 text-right`}>Max Late</th>
                </tr>
              </thead>
              <tbody>
                {filteredBranch.map((r) => (
                  <tr key={r.branch_code} className={TABLE_ROW}>
                    ...
                    <td className={`${TABLE_CELL} px-4 text-right`}>
                      <LateBadge minutes={r.total_late_minutes} />
                    </td>
                    <td className={`${TABLE_CELL} px-4 text-right text-zinc-400 tabular-nums`}>
                      {fmtMins(r.avg_late_minutes)}
                    </td>
                    <td className={`${TABLE_CELL} px-4 text-right text-zinc-500 tabular-nums`}>
          ...
          {filteredStaff.map((r, idx) => {
            ...
            return (
              <div
                ...
                className="overflow-hidden rounded-2xl border border-white/8 bg-white/[0.03] transition-colors hover:border-yellow-500/20"
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                    ...
                    <span className="text-[11px] text-zinc-500">
                      {r.late_days} day{r.late_days !== 1 ? "s" : ""} · avg {fmtMins(r.avg_late_minutes)}
                    </span>
                  </div>
                </button>
