...
function getApiBase() {
  ...
  if (configured) { return configured.startsWith("http://") ? configured.replace("http://", "https://") : configured; }
...
async function forward(req: NextRequest, params: { slug: string[] }, method: ForwardMethod) {
  ...
  if (!apiBase) return NextResponse.json({ detail: "API base URL is not configured." }, { status: 500 });

  const slug = (params.slug || []).map((part) => encodeURIComponent(part)).join("/");
  const search = req.nextUrl.search || "";
  const body = method === "GET" ? undefined : await req.arrayBuffer();

  const upstreamUrl = `${apiBase}/api/published/${slug}${search}`;
