...
export type ProcurementItemSearchRow = {
...
type Props = {
  onSelect: (item: ProcurementItemSearchRow) => void;
  market?: string;
  placeholder?: string;
  requestedBy?: string;
  pin?: string;
...
export default function ItemSearchInput({
  ...
  const [rows, setRows] = useState<ProcurementItemSearchRow[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const res = await procurementJson<ProcurementItemSearchRow[]>(
          `/api/procurement/items/search?${qs.toString()}`,
          { method: "GET" },
  ...
  function choose(item: ProcurementItemSearchRow) {
    applyValue(String(item.item_description || ""));
    setRows([]);
    setOpen(false);
    setActiveIndex(-1);
    onSelect(item);
      ...
      {open ? (
        <div className={`absolute z-20 mt-2 max-h-72 w-full overflow-y-auto ${GLASS_CARD} bg-neutral-950/95`}>
          {rows.length ? (
            rows.map((row, index) => (
              <button
                key={`${row.market || ""}:${row.item_description}:${row.invoice_date || ""}:${index}`}
                type="button"
              ...
              >
                <div className="text-sm font-semibold text-white">{row.item_description || "-"}</div>
                <div className="mt-1 text-xs text-zinc-500">
                  {[row.supplier_name || "-", money(row.unit_price, row.currency), row.unit || "", row.invoice_date || ""].filter(Boolean).join(" · ")}
                </div>
              </button>
            ))
          ) : (
            <div className="px-4 py-8 text-center text-sm text-zinc-500">No results yet — data will appear after invoices are imported</div>
          )}
        </div>
      ) : null}
    </div>
