...
14. **公開シフトの「最新」判定は `shift_published_rows.updated_at` を使う — `shift_published_versions.published_at` 禁止** → Manual Shift の編集は `(city, branch_code, week)` の **既存** version 行に行を追加するだけで `published_at` を更新しない。そのため `published_at` で並べると「今直した行」が「後から publish された別ブランチの古い行」に負ける。`shift_published_rows` を複数バージョン跨ぎで読む箇所は必ず `DISTINCT ON (staff_name, work_date) ... ORDER BY r.updated_at DESC, v.published_at DESC` にする。（2026-08 Patrick 8/20 の DTR が 9:00-18:00 のまま直らない問題で発生）
