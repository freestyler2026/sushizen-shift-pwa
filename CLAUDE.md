...
93. **`waitFor` の予算は vitest の `testTimeout` ではない — 既定は1秒で、CIでは足りない** → 2026-09-05、Frontend Tests が毎日数回落ち、そのたびに失敗メールが届いていた。`vitest.config.ts` に `testTimeout: 20000` があるので時間は足りているつもりでいたが、**`waitFor` は testing-library 自身の `asyncUtilTimeout`（既定1000ms）で動く**。`tests/admin/admin-page.test.tsx` の76箇所のうち timeout を明示していたのは3箇所だけだった。
    - **落ちたのは毎回1ファイルだけ（69/70成功）で、落ちるテストは実行ごとに違う。** これは「バグ」ではなく予算不足の形。同じテストが16秒後の実行では通っていた。
    - **数字で確かめる。** 該当アサーションはローカルで 19〜49ms。CIは70ファイルが2コアを共有するので30倍以上に伸び、1秒を超える。**通常35msの処理に1秒の予算**という比率だと分かって初めて、閾値の問題だと確定できる。
    - 対策は `tests/setup.ts` で `configure({ asyncUtilTimeout: 5000 })`。**本物の失敗は5秒後に同じように落ちる**ので、検知力は変わらない。
    - ⚠️ **緑と赤が混ざる通知は、赤を読ませなくする。** 毎日数通の失敗メールが「またあれか」になると、教訓47（成功を返す取込ジョブ）の逆向きの同じ失敗になる。フレークは直すか、テストを消すかのどちらかで、放置は選択肢にならない。
