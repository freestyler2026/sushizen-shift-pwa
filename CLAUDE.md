...
>
> **⚠️ 何かを断定する前に、必ず読む4節（この文書内）:**
> 「🔍 事実として報告する前に、反対側を数える」/「📏 大規模コードベースでの作業規律」/
> 「🧱 原因は『最初に見つかった説明』で止めない」/「❓ 質問する前に、質問の前提を検証する」。
> **ミスインフォの大半は、能力ではなく〈調べた範囲が狭かった〉ことで起きている。**
...
├─ **発注・PO・受領・Direct Purchase**   → **docs/procurement/CK_SUPPLIER_ORDER_MAP.md（必読・他を読む前に）**
...
| `docs/procurement/CK_SUPPLIER_ORDER_MAP.md` | **発注周りに触る前に必読。** purchase_type 5種の違い・PO と受領の粒度のずれ・Overdue 389件の68%がノイズである理由・マニラとドバイが正反対であること。`python3 scripts/verify-procurement-doc.py` がコードとの一致を検査する |
...
| Payroll系ページ（Manila Payroll / Dubai Payroll / **How Pay Is Computed** / Adjustments / Transactions / Loans / Leave Salary / Staff Profiles / Gov Tables / DTR Upload / Allowances / Remittances / My Pay / Inquiries） | **Payroll Manual** | `docs/manuals/payroll-manual.html` |
...
## 🕐 時刻は必ず店舗の時計で — ドバイとマニラで食い違わせない

**Herokuは UTC。マニラは +8、ドバイは +4。どちらも夏時間が無いので固定。**
都市は2つしか無いのだから、この問題をこれ以上発生させないこと。

**時刻がずれると、データの意味が失われる。** 実際に起きたこと:

| 起きたこと | 原因 |
|---|---|
| 食品安全のレポートが**上部17:34・事案欄09:34**と同じ瞬間を二重表示 | `incident_datetime`（TEXT）に `utcnow()` を書き、人の入力は現地時刻だった |
| マニラの **00:00〜08:00 の出来事が全部「昨日」**に入った | `closed_at::date = CURRENT_DATE`（UTC） |
| 懲戒通知の自動生成が**AE/PH に同じUTC日付**を使用 | `as_of = utcnow().date()` を市場ループの外で1回 |
| 7月の納品が**9月の事案**として記録 | `incident_datetime` に確定時刻を入れていた |

### 規則

**`app/city_time.py` を使う。** 自前で `utcnow() + timedelta(hours=8)` を書かない。

```python
from app.city_time import city_today, city_now, city_stamp, to_city, city_tz
city_today("manila")              # 店舗の「今日」
city_stamp(ts, "dubai")           # UTCの瞬間 → その都市の壁時計（TEXT列用）
```

**SQL では両辺を変換する。片方だけでは合わない。**
...
1. **タイムゾーンの印が無い文字列を保存して、画面が `new Date()` で描く。**
   `utcnow().isoformat()` は `Z` が付かないのでブラウザは**現地時刻として解釈**する。
   保存するなら**現地時刻で保存**するか、**`+ "Z"` を付けて UTC と明示**する。混ぜない。
   ⚠️ **人が `datetime-local` で入力した値は既に現地時刻。** その列に UTC を書くと意味が混在する。
2. **市場・都市ごとのループの外で「今日」を1回だけ計算する。**
   2都市に1つの日付は、定義上どちらかで間違っている。**ループの中で都市ごとに求める。**
...
### SQL では `store_today(city)` を使う — CURRENT_DATE は書かない

DB側に関数を1つ置いた（`ensure_store_today_sql()` が起動時に作る）。
`app/city_time.py` と同じ規則で、**NULL や未知の都市はマニラ**に落ちる。

```sql
WHERE scheduled_date < store_today(s.city)        -- 期限判定
(store_today(a.city) - a.applied_date) AS days    -- 経過日数
date_trunc('month', store_today(city))            -- 月の境界
```

2026-09-13 に **49箇所**を移した。移した対象は3つの型だけ:

| 型 | 例 | 直さなかった理由 / 直した理由 |
|---|---|---|
| **期限・今日の判定** | `scheduled_date < CURRENT_DATE` | 現地0-8時は「昨日」で答える。**開店シフトが働いている時間そのもの** |
| **経過日数の表示** | `(CURRENT_DATE - applied_date)` | 同じ時間帯だけ1日少なく出る |
| **月の境界** | `date_trunc('month', CURRENT_DATE)` | **毎月1日の朝は「先月」が出る** |
| 期間フィルタ | `>= CURRENT_DATE - 90 days` | **触っていない。** 8時間のずれは取得する履歴の範囲を変えない |
| `DEFAULT CURRENT_DATE` | テーブル定義 | **触っていない。** 12個すべて、INSERT が日付列を明示するので発火しない（確認済み） |

⚠️ **都市列が無いテーブルに都市を発明しない。** `proc_emergency_whitelist`・
`admin_channel_usage`・`renewal_custom_alerts` は都市を持たないので CURRENT_DATE のまま。
マニラ固定にすればドバイ側が別の向きにずれるだけで、UTCより正しくはならない。

⚠️ **`store_today(%s)` を足したらパラメータも足す。** 私は1箇所で忘れ、
`tuple index out of range` で請求書の鮮度クエリを落とした。**compile() は通る。**

### SQLを直したら、本番でその関数を実際に呼ぶ

`compile()` は構文しか見ない。列名の誤り・プレースホルダ数の不一致は**実行して初めて出る**。
変更した行から**囲っている関数名を機械的に列挙**し、読み取り系を one-off dyno で全部呼ぶ:

```bash
git diff -U0 | grep '^@@'     # → 変更行 → その行を含む def を特定
```

⚠️ **one-off dyno は「デプロイ済みのコード」を動かす。** ローカルの変更は入っていないので、
**デプロイしてから実行する**。この順番を間違えると「変更前のコードが通った」だけで安心する。
2026-09-13 はこの検証が、自分の入れた不具合1件と**既存の不具合1件**
（`list_overdue_deliveries_admin` が存在しない列 `r.request_status` を参照して毎回失敗）を拾った。
...
**2026-09-12、1つのやり取りの中で同じ型の誤りを5回出した。** 全て「確かめる手段が手元にあるのに、確かめる前に報告した」もの。

| 報告した内容 | 防ぐのに必要だったこと |
|---|---|
| 「13名はSSS番号が未登録だから外れている」 | **記入済み49名も数える**（6名が同じく未登録だった） |
| 「WISP対象者だから外れている」 | **記入済み側のWISP保有者を数える**（43名中30名が記入済み） |
| 「YuriにはPayroll権限がない」 | **`_salary_view_mode` を読む**（HQはロールで通る） |
| 「少額7名は全員が部分月」 | **7名全員の勤務日数を出す**（1名は23日勤務） |
| 「13名は全員バックオフィス」 | **`branch_code` を読む**（2名はCUB所属） |

### 規則

**ある集団Aに共通する特徴Xを見つけたら、報告する前に「Xを持つが集団Aに入らない人」を数える。** 数が0でなければ、Xは説明になっていない。

- 「AだからBだ」と書く前に、**Aでない側にもBがいないか**を同じクエリで数える
- ロール・権限の判定は、**権限リストだけでなくロールによる分岐も読む**（HQ・ADMINはロールで通ることが多い／教訓25・32）
- 「全員が〜」「1人もいない」と書く前に、**全件を出して目で確認する**。標本や上位数件で言わない
- 仮説の段階では**仮説と書く**。「〜が原因です」と「〜が原因ではないかと考え、確認します」は別物

### なぜ重要か

ユーザーは受け取った内容を前提にスタッフへ指示を出す。**誤った前提で現場が動くと、こちらが節約した検証時間の何倍もの労働時間が失われ、しかも信用が落ちる。** 検証はこちらで完結させる。

---

## 📏 大規模コードベースでの作業規律

このリポジトリは複数モジュール（給与、勤怠、経費、通知、採用、日次P&L、CK振替、ケース管理、
在庫、調達）を持ち、1セッションで全体を読み切ることはできない。以下は、**全体を把握していない
状態で、誤った前提のまま進む事故**を防ぐための規律である。

### 1. 読む前に語らない

**既存のコード・データ・仕様について述べるときは、必ず該当箇所を読んでから述べる。**

- 既存機能の動作を説明するとき → ファイルを開き、`path:line` を引用する
- 既存データの状態を前提にするとき → 実際にクエリを実行し、結果を示す
- 既存テーブルの列の意味を前提にするとき → スキーマとサンプル行を確認する

「〜のはずです」「通常は〜」「一般的に〜」で既存の状態を語らない。
読んでいなければ**「未確認」と書く。**

### 2. 調査範囲を明示する

**作業報告には、読んだものと読んでいないものを分けて書く。**

```
読んだ:   app/manila_payroll_engine.py, db.py:100-250, migrations/0041
読んでない: services/ar_drive.py（Drive連携の詳細は未確認）
前提:     pos_sales_channel_daily に全チャネルが入っていると仮定（未検証）
```

**推測を推測と明示すれば、それはミスリードにならない。
明示せずに断定した推測が、ミスリードになる。**

### 3. 調査と実装を分ける

以下に該当する変更は、**実装前に「現状報告」を出し、承認を待つ。**

- 金額の計算に影響する変更
- 法令・規則に関わる変更
- 既存テーブルのスキーマ変更、または既存データの意味の解釈に依存する変更
- 複数モジュールにまたがる変更
- 本番データの削除・再構築を伴う変更

```
## 現状（読んだ結果）
- <ファイル・テーブル・実データから確認した事実>
## 変更案
- <何を、どう変えるか>
## 影響範囲
- <他に触れるモジュール、変わる挙動>
...
該当しない小さな変更は、そのまま実装してよい。

### 4. 既存の資産を先に探す

**新しいテーブル・サービス・ユーティリティを作る前に、同じ目的のものが既にないか探す。**

```bash
grep -rn "<関連キーワード>" app/ src/ --include="*.py" --include="*.tsx" -l
```

過去の事故：`ar_payouts` の按分で日次売上を作ろうとしたが `pos_sales_channel_daily` が既にあった。
`store_code=''` で集約しようとしたが店舗別マッピング辞書が既にあった。
**同じ目的のものが2つできると、必ず数字がずれる。**

### 5. モジュールの README を最初に読む

各モジュールのルートに `README.md` を置く（なければ作る）。内容：

- このモジュールが何をするか（3行以内）
- 主要テーブルと、**各列の意味で誤解しやすいもの**
- 主要な関数・エンドポイント
- 既知の制約・未解決の設計課題
- 他モジュールへの依存

**そのモジュールに触れる前に README を読む。** 全コードを読む必要はないが、README は読む。
無い・古い場合は作業の最初に更新する。**README の更新は実装の一部で、後回しにしない。**

### 6. 変更したら README とテストを更新する

**コードだけ変えて README を古いままにしない。** 次のセッションが古い README を読んで、
誤った前提で作業を始める。

**金額・日付・状態遷移に関わるロジックを変えたらテストを追加・更新する。**
テストが無い領域を変更する場合は、**変更前に現状の挙動を固定するテストを先に書く。**

### 7. 数字を扱うときの規律

- 計算式を変える前に、**変更前の結果を記録する**（特定の入力に対する出力）
- 変更後、同じ入力で比較し、**差分を報告する**
- 「正しくなったはず」ではなく「**入力Xに対してYがZになった**」と報告する
- 集計値には**内訳と件数**を必ず添える。合計だけ出さない

過去の事故：Jaybert の初回締めで2日分の給与から半月分の法定控除を引いた。
「レートの半月分」と「実報酬」を区別せずに計算していた。

### 8. 本番データへの破壊的操作

DELETE / TRUNCATE / DROP / WHERE 無しの UPDATE / キャッシュの全再構築。

- **必ず事前にバックアップ**（該当テーブルのダンプ、または件数と合計値の記録）
- **トランザクション内で実行**し、件数が想定と違えばロールバック
- 実行前に「**何件消え、何件入り、合計値がどう変わる見込みか**」を報告する
- 削除→再投入は途中で止まるとデータが消えたまま残る。
  **削除ではなく条件付き UPSERT** を使えないか先に検討する

### 9. セッションの切り方

- **1セッション1タスク。**「ついでに」を受けない
- 長いセッションでは冒頭で読んだファイルが編集で変わっている。**再度触るファイルは再度読む**
- **タスクの途中で前提が崩れたら、そこで止めて報告する。辻褄を合わせて進めない**

### 10. 報告のとき

- 「完了しました」の前に、**何を変えたか／何を変えていないか／何を確認していないか**を書く
- 見つけた問題は依頼されていなくても報告する。**ただし修正は依頼を待つ**
- 自分の過去の発言・実装が誤っていたと分かったら、**最初にそれを言う。説明は後**

---

## 🧱 原因は「最初に見つかった説明」で止めない — 経路を最後まで辿る

**2026-09-22、同じ問い（CK Par Level にドリンクが出ない）に3回答え、2回間違えた。**
どの回も、調べた範囲では正しかった。**調べた範囲が間違っていた。**

| 回 | 私の答え | 実際 |
|---|---|---|
| 1 | 「仕入先 SUY Sing (Drink) が無効化されたのでカタログ検索に出ない」 | 正しいが**症状の説明で、原因ではない** |
| 2 | 「Restaurant Depot にカタログ登録すれば直る」 | **直らない。**6品は既に par リストにあり仕入先も登録済みだった |
| 3 | 「22行が単位不一致」 | **20行。**`POR` と `Portion` は同じ単位で、4行は私の誤検知 |

真因は**棚卸しが BTL・par が CASE で、引き算が単位を見ていなかった**こと。
1回目の答えの1階層下にあり、**クエリ1本で出た。**

### 規則：ゲートを全部並べてから原因を言う

「なぜ X が起きなかったのか」に答えるとき、**X に至る関門を全て列挙し、1つずつ判定してから**
原因を述べる。最初に見つかった不成立で止めない。**複数が同時に不成立のことが普通にある。**

この画面の例（発注に載る条件）:

```
par 設定あり → 仕入先あり → 棚卸しに名前がある → 単位が比較できる
  → par − 在庫 > 0 → カタログに単価がある
```

私はゲート6（カタログ）で止めた。実際に塞いでいたのはゲート4（単位）だった。

- **「〜だから出ない」と書く前に、他のゲートも全部評価する**
- 報告には**ゲート別の判定**を書く（「6品はゲート1〜3を通り、ゲート4で落ちている」）
- **1件で確かめて一般化しない。**全件をゲートに通し、分布を出す

### 規則：自分の検出ロジックにも誤検知がある

「22行が不一致」と報告しかけたが、4行は `POR` vs `Portion` という**同じ単位の別綴り**だった。
そのままデプロイしていたら**生産計画の4品を壊していた。**

- **異常として列挙したものは、1件ずつ「本当に異常か」を見る。**件数だけ見ない
- 同じセッションで `JOIN ... ON a OR b` が別行を拾って1件を誤って混入させ、
  同名2行のテーブルで件数を1多く数えた。**照合クエリは本番コードと同じ引き方で書く**
  （`_stock_for` が名前A→名前Bの順で引くなら、検証も同じ順で引く。SQL の OR で代用しない）

---

## ❓ 質問する前に、質問の前提を検証する

**間違った前提の質問は、質問しないより悪い。** 受け取った側は、前提が間違っていることを
自分で調べて気づき、訂正のプロンプトを書くことになる。**こちらが節約した検証時間の
何倍もの時間を相手から奪う。**

2026-09-22 の実例：「Restaurant Depot にドリンク9品を登録してよいか」と聞いた。
**前提（登録すれば発注に載る）が間違っていた。**オーナーは「これで直るのか」と
問い返す必要があり、そこから3往復かかった。

### 規則

1. **質問文に前提を明示し、その根拠を1行で添える。**
   - ❌「9品を登録してよいですか」
   - ⭕「9品を登録すれば Add Item に出ます（`catalog-items` の active フィルタで確認済み）。
     ただし**発注に載る条件は別**で、そちらは未確認です」
2. **「これをやれば直るか」を自分に問い、答えられなければ質問を出さない。**
   出す前に、直ることを実データで確かめる
3. **聞く相手を間違えない。**コード・DB・git log・一次資料で答えが出るものは聞かない
   （既存の「判断を渡さない」節のとおり）
4. **1通の質問は1つの決定に絞る。**「AとB、どちらも決めてください」は、
   片方の前提が崩れると全体が止まる

---

## 🔬 精度を上げるための手順（速度より精度を優先してよい）

オーナーの明示指示（2026-09-22）: **コストが増えてよいので、ミスインフォを減らし
開発速度と精度を上げる。** 以下はそのための具体手順であり、省略しない。

### 1. 事実の主張は、独立した目で再検証する

**Agent ツール（`Explore` / `general-purpose`）を、以下の用途に限って使ってよい。**
（この文書による明示的な許可。他の用途には使わない）

| 使ってよい場面 | 渡すもの |
|---|---|
| 「なぜ X が起きないか」の原因を報告する前 | 私の結論とゲート一覧。**反証を探させる** |
| 本番データの集計を報告する前 | 私のクエリと結果。**別の引き方で同じ数字が出るか** |
| 複数ファイルにまたがる影響範囲の確認 | 変更する関数名。**呼び出し元を全部列挙させる** |

**「私の結論はこうです。間違っているなら指摘してください」と渡す。**
同意を求めない。**反証が出たら、出た側を先に確かめる。**

### 2. 報告の前に、この4つを自問して書き出す

```
① この主張は、どのファイル/クエリのどの行で確かめたか
② 反対側を数えたか（Xを持つが集団Aに入らないものは何件か）
③ 他に同じ結果を生む経路はないか（ゲートを全部評価したか）
④ この主張が間違っていたら、相手は何を失うか
```

④が「スタッフへの指示」「発注」「給与」なら、**①〜③を満たすまで報告しない。**

### 3. `/code-review ultra` を使う（オーナーが起動する）

金額・法令・複数モジュールに関わる変更は、マージ前に
```bash
/code-review ultra
```
を実行してもらう。**これはオーナーしか起動できない**ので、
**該当する変更を出したら、私から「これは ultra レビュー対象です」と申告する。**

### 4. 深く考えさせたいときの合図（オーナー側の操作）

原因調査・設計判断・法令解釈など、間違いのコストが高い依頼では、プロンプトに
**`think hard` / `ultrathink`** を入れると思考量が増える。
**「調べて」ではなく「ultrathink で調べて」**と書けば、私が浅い層で止まる確率が下がる。

### 5. 過去の事故の記録は、事故のたびに増やす

**この規律はミスリードが出るたびに見直す。**新しい型が出たら該当節に
「過去の事故」として日付つきで追記する。**書かなければ同じ型で再発する。**

---

## 🧭 判断を渡さない — 調べて、決めて、理由とともに報告する

**このOSはオーナー1人で開発している。** 規模はすでに1人で全部を把握できる大きさを超えている。
そこへ判断を戻すのは、支援ではなく**負荷の付け替え**になる。

2026-09-12 のSSSの一件で、同じ失敗を3つの相手に対してやった。**どれも「自分で結論に
到達する作業をしなかった」という一つの失敗の別の顔だった。**

| 渡した先 | 実際にやったこと | 本当はどうすべきだったか |
|---|---|---|
| スタッフ | 「ポータルの数字を送ってください」と聞いた | ポータルが出す額は公開表の決定的な計算。**ログイン不要で自分で出せた** |
| オーナー | 「AとB、どちらにしますか」と2択で戻した | IRRとR-3の原文を読めば**1時間で決着した**（実際そうなった） |
| 会計事務所 | 「見解が要る」として教訓36に3回保留した | 保留の間、**毎月誤った基礎で申告し続けていた** |

### 既定の動き方

**調べる → 決める → 「こう判断しようと思う、理由はこれ」と報告する。**
反対されたら変える。これが既定で、質問は例外。

- ❌「AとBどちらにしますか」
- ⭕「Aにします。理由はIRR Rule 31 §1がこう書いているからです。違えば言ってください」

**判断の材料は、聞く前に自分で取りにいく。**

| 探せる場所 | 例 |
|---|---|
| 本番DB | 誰が何件やっているか、実際にいくら払ったか、フラグと実態のズレ |
| コードとgit log | その仕様がいつ・なぜそうなったか（**昨日変わっていることがある**） |
| 一次資料（法令・様式・公式サイト） | 制度の規則。**二次的な解説記事と食い違ったら条文が勝つ** |
| 過去のやり取り・CLAUDE.md | 既に決めた運用（**決めたことを作り直しの過程で落とさない**） |

### 本当に聞くべき3つだけ

1. **取り消せない外向きの行為** — 送信・公開・納付・支払い・削除
2. **金額や人事が動く変更** — 給与額が変わる再計算、権限の剥奪、在籍ステータス
3. **会社の方針で、どの文書にも書かれていないもの** — 「無給休職の人に半月分を払うか」など。
   ⚠️ ただし**「制度がどう定めているか」は方針ではない。** それは調べる対象。取り違えない

### 「専門家に聞く」に逃がす前に、1時間だけ自分で当たる

- 一次資料（法令・IRR・様式の記入要領・公式サイト）に当たる
- **反対説を探す**（片側だけ数えない）。根拠のある反対説が出てこなければ、それは論点ではなく答え
- それでも割れたら、**争点を1つの二択に圧縮して**、金額差をつけて渡す
  （「欠勤を引くか引かないか、差は₱6,450」— ここまで絞れば専門家は即答できる）

### スタッフに投げる前の一線

**スタッフに聞いてよいのは「その人しか知らないこと」だけ。**
書類に書いてあること、DBに入っていること、計算で出ることを聞くのは、
**こちらの手間を相手の労働時間に付け替えているだけ。**

そして**検証していない内容をスタッフに送らない。** 誤った指示は、相手の作業時間を奪ったうえ、
次に正しいことを言っても通らなくなる。SSSの件では訂正を3回送り、そのたびに信用を削った。
**送る前に「これは確かめた事実か、私の推論か」を1行ずつ判定する。**

---

## 🔒 給与ファイルを渡す前に、宛先で絞る — マスクはエクスポートで無効になる

**2026-09-12、62名分のSSS明細を給与担当スタッフに渡す直前だった。** バックオフィス11名の
報酬額が入っており、その中に**受け取る本人（Marithel）と、最高額の Peter Villafuerte ₱63,711**
が含まれていた。前半のやり取りで「HQの意向でバックオフィスは外す」と確認済みだったのに、
62名で作り直した時点でその判断が消えた。ユーザーの指摘で止まった。

**マスクは壊れていない。迂回された。** `_salary_view_mode` / `_mask_salary` はAPI層で効くが、
**one-off dyno から psycopg2 で直接読んだデータには一切かからない。** 私がファイルを作った
瞬間に、マスクの仕組みは丸ごと無効になる。教訓67・68で作り込んだ保護が、
エクスポート1回で意味を失う。

### 規則

- **人別の金額を含むファイルは、作る前に宛先を決め、その宛先が見てよい人だけで作る。**
  作ってから絞るのではない。作った時点で流出経路が存在する。
- **ファイル名に宛先を入れる**（`..._store_51.xlsx` / `..._backoffice_11.xlsx`）。
  `FINAL` のような名前は、中身が誰向けかを消すので使わない。
- **渡す直前に検査を通す**（注意書きでは同じことが起きる）:
  ```bash
  python3 scripts/check-export-audience.py <file> --for "<誰に渡すか>"
  ```
  人別の金額列を検出し、**含まれる拠点と氏名を列挙して exit 1 で止める**。
  `--for` を付けても、拠点一覧に `BO` が混ざっていればその場で見える。
- **マニラのバックオフィスの給与は、店舗の給与担当に見せない。** 分けて Yuri（HQ）に渡す。
  これは過去に一度決めた運用であって、毎回思い出すものではない。**検査で止める。**

---

## ✍️ スタッフへの返信文を書くときの原則

**書き手はオーナー本人。** 返信案は常に「オーナー → スタッフ」の声で書く。

### 過剰に非を認めない

不具合の原因を説明するとき、「これはこちらのミスです」「私の責任です」「ご指摘いただかなければ気づけませんでした」を積み重ねない。**1通につき0〜1回まで。**

**理由:** このOSはオーナーとClaudeの2人で開発している。この規模で不具合が出るのは当然で、プロのエンジニアでも同じ。謝罪を重ねると、実際には大量の労力を投じている事実が消え、指示が通らなくなる。**スタッフに舐められる。**

### 書き方

| やること | やらないこと |
|---|---|
| 事実を一度だけ書く（「Xという仕様だったのでYになっていた。直した」） | 原因の自己批判を前に出す |
| 修正した事実と、次からどうなるかを先に書く | 「私のミスです」を繰り返す |
| 感謝は短く1回（「ありがとう」で足りる） | 「ご指摘いただかなければ…」と持ち上げる |
| 依頼は依頼として書く（「〜を教えてください」） | 許可を請う形にする（「〜していただけますでしょうか」） |

### 敬語の水準

- **ます/です** — ております／いただきます／おかけいたします は使わない
- 締めの「引き続きよろしくお願いいたします」は不要
- スタッフからの確認に、いちいち受領の返事を足さない

詳細は memory の `feedback_reply_style.md`。

---

## 🔧 依存が無いときは、回避せずインストールする

2026-09-12、xlsx の `recalc.py` が `soffice not found` で落ちたとき、私は
「LibreOffice が無いので再計算できません。数式は単純なので Excel で開けば計算されます」
と報告して先に進んだ。ユーザーがその場で入れて指摘した —
**「すぐに回避するのではなくインストールを選んでください」。**

**回避すると、検証が一段まるごと抜ける。** このときも再計算を通して初めて
「Match 列が入力に対して本当に動くか」を確かめられた（975→match /
487.50→−487.5 / 1000→+25）。回避したままなら、**動作未確認のファイルを
「確認しました」と渡していた。** 教訓56（最後は必ずボタンを押す）と同型。

- 足りないのがコマンド1つなら、まず `brew install` / `pip install` / `npm i` を試す
- 入れられなかったときだけ、**抜けた検証の名前で**報告する
  （「数式が評価されることを確認していない」）。「無くても大丈夫です」と書きそうに
  なったら、それは検証を1つ捨てているサイン
...
36. **【2026-09-12 決着】SSSの算定基礎は「その月に実際に受け取った報酬」** — 契約月給ではない。
    何度も保留にしてきた論点をSSSの一次資料で確定させた。**4資料すべてが同じ方向を指し、根拠のある反対説は存在しない。**

    | 出典 | 文言 |
    |---|---|
    | SSS公式サイト | MSCは "based on the **total actual remuneration** from employment" |
    | IRR SEC 6 | COMPENSATION = "All **actual** remuneration for employment"（**Overtime pay を明示列挙**） |
    | IRR Rule 31 §1 | 拠出金は "corresponding to ... compensation or **earnings during the month**" |
    | SSS R-3 様式 項目10 | 事前印字は "must be corrected corresponding to the **actual income** ... for the period" |

    - **無給欠勤・遅刻・早退は基礎から引く。** 受け取っていない給与は actual remuneration ではない。
      月の全部が無給なら拠出金ゼロ（引く原資が無い）。
    - **寮費・ローン・源泉などの回収は引かない。** 報酬の減額ではないため（旧36の指摘はこの扱いで正しかった）。
    - **OT・深夜手当・休日割増・賞与（Christmas bonus除く）・COLA・月次手当は含む。** 「OTは含まない」と書く
      二次情報があるが、IRR SEC 6 の列挙と矛盾するので採らない。
    - **ポータルが出す数字は「前回申告のまま」の初期値**であって正解ではない。訂正が employer の義務。
      ⚠️ **2026-09-11 に「ポータルと合わない」を理由に基礎を契約月給へ変えたのは誤り**（`e3aeb677`）。
      合わなかったのは初期値との差で、それは訂正すべきものだった。**戻すこと。**
    - 8月実績: 実収入基礎 ₱62,300 / 欠勤を引かない場合 ₱68,750 / 契約月給基準 ₱69,000（62名・EE分）。
    - ⚠️ **この論点は「会計事務所待ち」として3回先送りされた。** 先送りの間、毎月誤った額で申告し続けていた。
      **一次資料に当たれば1時間で決着する論点を、判断待ちとして寝かせない。**

36b. **（旧36・解決済みの記録）統計控除（SSS）の算定基礎に Adjustment が入っていた** → `compute_payroll_run` は「基本給・OT・ND・遅刻/早退/欠勤」→「Adjustment」→ その合計を `monthly_gross` としてSSS表を参照する。寮の電気代 −272.55 を入れるとSSSが 450.00→437.50 に落ちる（ブラケット降格）。フィリピンの制度上SSSは報酬に対する掛金で、寮費の回収は報酬の減額ではない。
...
54. **一括処理の「効かないこと」を本番で検証するときに、極端な引数を渡してはいけない** → 2026-09-02 に私が実際にやった。`expire_stale_management_tasks` が `kpi_*` を除外するようになったかを確かめるため `days=0`（＝全件が期限切れ扱い）を本番で実行し、**未送信のopenタスク89件（ドバイ79・マニラ10）を実際にクローズした**。検証したかったのは「kpi_が0件であること」で、その結論自体は正しく出たが、代償が本物の89件だった。
    ...
    - ⚠️ **後片付けの DELETE を前方一致で書かない（2026-09-14 実際に発生）。** 検証で作った
      `ZZ Resume Check` を消すのに `WHERE full_name LIKE 'ZZ %'` としたところ、ユーザーが
      9/10 に作っていた **`ZZ TEST — do not process (Yukihiro)`** も一緒に消えた。
      **自分が作った行は id が分かっている。** id で消す。パターンで消してよいのは、
      **先に SELECT して出た行数と中身が、作った覚えのあるものと一致したとき**だけ。
      ⚠️ **「直近N時間」も同じ罠（2026-09-15 に再発）。** 検証の後片付けを
      `created_at > NOW() - INTERVAL '3 hours'` で書いたところ、**同じセッション中に
      本番で行った一括発行の `invite` 記録**が1件巻き込まれた。時間の窓は
      「自分が作ったもの」を指していない。**削除の前に1件ずつ表示し、
      自分が作った覚えのある id だけを消す。**
      復元できたのは `hr_applicants.booking_invited_at` が同じ時刻を
      マイクロ秒まで保持していたからで、**運が良かっただけ**。
      応募者行はバックアップから戻せたが、紐づいていた音声枠は戻せなかった
      （`hr_applicants` しか退避していなかった）。**退避は、消える可能性のある表すべてに掛ける。**
...
96. **「所属」で束ねた一覧に「その日の実績」を描くと、見出しが嘘になる** → シフトのExcel出力はタブを`staff_master`の所属店舗で作り、そこに公開シフトの実際の勤務店舗を描いていた。Al Barsha のシートに、9月まるごと Business Bay で働く Padam が載る。**唯一の手掛かりがバーの色**だったため、店舗マネージャーは「色を確認してください」とアナウンスして運用していた。この表はDiscordに画像で貼られ、スタッフが**どの建物に行くかを決めるために見る**。（2026-09-07 修正）
    - 直し方は色や注記を足すことではなく、**束ね方をその日の事実に変える**こと。人はその月に働く各店舗のタブに載り、日はその店舗で働く日だけ描く。`Total h` は行に対する数式なので、他店の日を空欄にすれば自動的に「そのタブでの時間」になった。
    - **分割シフトの日は当日を丸ごと描く**（片方の店舗だけ描くと「いつ抜けるか」が消える）。Rafael の 9/1 は JLT 12-16 → AM 18-22 で、両店舗のタブに全体が出る。
    - ⚠️ **「所属と実勤務がずれている人」を数えると表記ゆれで水増しされる。** 生データで30名出たが、`AL BARSHA`と`AB`のような同一店舗の別表記を正規化すると**実際は5名**だった。件数を報告する前に正規化を通すこと。
    - ⚠️ **セルを空にする変更は、なぜ空なのかを書かないと「無い」と読まれる。** タブを実勤務で分けた結果、Rafael の Business Bay 行は30日→8日になり、**「シフトが無い」と報告された**（実際は4タブに分散）。空欄は「予定なし」の意味を既に持っているので、`→ Al Barsha` と書いて全日を説明した。**正しくしたつもりで読めなくしていた。**
    - ⚠️ 同じ修正で `Total h` も壊していた。分割勤務の日を丸ごと描いたため、合計が他店の区間まで数え、CKのシートが「Hayat は174時間」と表示した（実際は20時間）。**1つの変更が、別の場所の意味を静かに変える。**
    - 退職者が残る件は出力側の不具合ではない。名簿は `status='ACTIVE'` で正しく絞れており、**残っているのは退職処理が未実施のため**。出力を直しても消えないので、Staffページで SEPARATED にする運用が要る。

97. **一覧に行を流し込む改善は、その行が何かを起こす条件を満たしているか先に見る — 満たしていなければ「入れたのに何も起きない」になる** → CK Par Level に「カタログの149品も入れられないか」という話になったが、発注ドラフトは `par − 在庫 > 0` の行しか拾わない。**par が空の行は絶対に発注に出ない。** その実験は既にドバイで走っていて、`ingredient_master` から一括投入した **supplier 291行が全て par 空・仕入先空**、1件も発注を生んでいない（マニラは124行中121行に par があり、1品ずつ人が入れた）。流し込めばリストは269行に膨らみ、発注できる品は1つも増えなかった。**一括投入の前に「投入した行が下流の条件を通るか」を1本のクエリで数える。**
    - 代わりに入れたのは検索ピッカー（発注時にカタログ全件へ届く／リストは124行のまま）。**「一覧に載せる」と「その場で選べる」は別の解で、後者は下流の条件に依存しない。**
    - ⚠️ **同じテーブルに書き込むピッカーを2つ作るなら、同じリストを出すこと。** `proc_request_items` には `/store/purchase`・BO編集画面・今回のCK発注モーダルの3経路が書く。フィルタ（`order_type IN ('supplier','ck_wh_to_supplier')`）を揃えずに片方だけ広げると、どちらの画面から入れたかで通る品が変わる。
    - ⚠️ **そのフィルタが何を落としているかを画面に返す。** ドバイはカタログ573行中**561行が `order_type` 空欄**で全部落ちる。件数を返さずに空振り時「Cost Calculation に登録してください」と出すと、**既に登録済みのものを登録しろと言うことになる**（教訓21・91と同型）。落とした件数を返し、空状態に併記した。
    - **自由入力の穴は、塞ぐ前にどこから何件入っているか数える。** マニラの Direct Purchase 明細は90日で1,603行中**260行（16%）がカタログに無い名前**、うち76件は既存品の綴り違い（`Kraf`/`Kraft`、`Shichuan`/`Sichuan`、`coconut`/`coconuts`）。1画面だけ自由入力を断っても、残り2経路から同じ速度で入り続ける。

98. **単価の妥当性は単位ラベルではなく `line_total` の分布で決める** → `Beef Short Plate` の単価を「25kg@515 と 1box@515 のどちらが正か」と聞いたが、**行合計を出さずに聞いたので質問が成立していなかった**。19行を合計付きで並べると、**APPROVED 14件は全て 25〜26 × ₱490〜515 = 約₱12,250〜12,875**、`1 box @515`（＝₱515）の行は**1件も承認されていない**。つまり ₱515 は kg 単価で、box/block は25kg。単位ラベルは `kg` と `BOX` が同じ形の行で入れ替わっており、**ラベルは信用できないが数量と合計は一貫している**。
    - カタログ側は `unit='BOX'` で ₱400 / ₱515 を持っている（＝kg単価をBOX単価として登録）。**単位が違えば25倍ずれる**ので、価格の自動引き当てはここで嘘をつく。
    - `_price_for` が0を返したのは欠陥ではなく**ガードが働いた結果**。同名で価格が2つ（Meat Plus ₱400 / JB Frozen ₱515）あり、par 行の仕入先 `Happy Meat` がどちらとも一致しないので、他社の値段を当てずに0にした。**「価格が入っていない」と「価格を当てなかった」を混同しない。**
    - ⚠️ **社内の数え方の単位は、仕入単位に換算できないことがある。** CK はこの品を `Block` で数えるが、実績は 3・4・8・12・14・16… で**倍数関係が成り立たない**。私は「8の倍数だから1箱=8ブロック」と書きかけて、distinct を取って外れた。**換算率を主張する前に、全ての実測値を並べて割り切れるか確認する。**
    - 決着したのは `proc_receiving_items`（**納品9回すべて約25kg**）。発注も棚卸しも表記が揺れるが、**受け取った物の記録だけは物理量**なので、単位論争はここで決める。

99. **読み取りと換算を同時に頼むと、読めているのに答えを間違える — 文字は模型に、暦はコードに** → 請求書の日付を `YYYY-MM-DD` で返させていた。これは「文字を読む」と「どれが日かを決める」を同時に要求している。MIY Foodstuff は `5-Sep-26` と印字する。模型は末尾の26を日と取り、9月26日と3回答えた。「末尾の26は年だ」と指示を足すと、今度は1日と答えた。**どちらも書類に無い日付**で、写真を実際に開いて確認した。
    - 直し方は指示の追加ではなく**役割の分離**。`invoice_date_printed` に印字どおり写させ、`_parse_printed_date()` が暦を決める。順序が本当に曖昧な `05/09` はアップロード日で決着させ、決着できなければ**日付を作らずに注記を残す**。
    - ⚠️ **「全書類は2026年」という指示が、この誤りを可能にしていた。** 年を与えた結果、末尾の26が余り、日に回された。**制約を与えると、その制約が埋めた枠の中身が動く。**
    - ⚠️ **修正の検証は、変化した方向が危険な1件で行う。** 391・516（アップロード日に寄った）は正解だったが、45は逆にアップロード日から離れ、写真を見ると**新しい答えの方が誤り**だった。**直後の集計が改善していても、悪化した側を1件開くまで直ったと言わない。**
    - 反復読み取りの実測: 12件中11件は印字文字列まで完全一致。唯一の差 `19/08/2026` vs `8/19/2026` は順序違いで、パーサが同じ日付に解消する。**ばらつきは小さく、パーサが吸収する。**

100. **確率的な抽出器を、既に正しい行に再実行してはいけない — 再実行は改善ではなく賭け** → 日付が妥当な範囲に入っている行まで再OCRした結果、id=45 が正しい `2026-08-20` から誤った `2026-07-26` に変わった。この行は3回読ませても安定して誤読する（金額 439.43 は毎回正確）ので、ばらつきではなく**固定的な誤読**であり、以前の実行がたまたま正しく読めていただけだった。
    - **再実行の対象は「妥当でない行」に限る。** 妥当な行を混ぜると、直る数と壊れる数が同時に増え、差し引きが測れなくなる。
...
101. **送信するBotを別アプリに移したら、その返信を聞いているリスナーも一緒に動かす** → 遅刻アラートは「Reply with: `I'll handle it`」と案内しながら、返信が1ヶ月間どこにも届いていなかった。**最初から壊れていたのではなく、途中で壊れた。**（2026-09-13 修正・`run_ack_listener()` を追加）
    ...
    - 実測は **`ack_method` 列で数える**。全ack 627件の内訳は **AUTO 619 / UI 5 / DISCORD_DM 3**。
      **Discordの返信で閉じたのは生涯3件、全て 2026-08-07〜08-13** — トークンを移した当日が最後。
      ⚠️ **私は最初 `acknowledged_by` の文字列で「人によるack 8件」と報告したが誤り。**
      5件は画面からの ack（`UI`・8/01・Yukihiro）で、Discord返信ではない。
      **判定用の列があるならそれを読む。表示名の文字列から主体を推測しない。**
      「acked 34/35（97%）」という率も嘘で、中身はほぼ全部 AUTO（期限切れ・本人が打刻）だった。
      **ack率ではなく ack した主体で数えること。**
...
103. **「期限切れか」を今日と比べると、確認が遅れただけで期限切れが生まれる — しかもその記録に人の名前が署名される** → CK納品の受領確定で `expiry_date < CURRENT_DATE` を満たす品を自動で `EXPIRED` にし、Incident Report を起票していた。**納品#82 は 2026-07-26 着・2026-09-05 確定**で、flagged 31件は**全件が納品日時点では期限内**（期限 07-30〜08-27）。同型が **7納品・77行**。（2026-09-13 修正）
    - 比べる相手は **`d.delivery_date`**。「渡された日に期限が切れていたか」が問いで、「確認が遅れた間に切れたか」は別の問題であり、送った側の食品安全記録に付けてよいものではない。
    - ⚠️ **`<input type="date" min=...>` はピッカーの制限でしかない。** 手入力は素通りし、この画面は `onChange` で即保存する。結果、**期限日が製造日より前**という物理的にありえない行が5件入った（Nobashi Shrimp: 製造 9/8・期限 7/9）。**年を 2028 と 2026 で打ち間違えた形。** サーバ側で `expiry <= production` を拒否し、エラー文に「ラベルの年を確認してください」と原因を書く。
    - ⚠️ **自動生成される記録の `reporter_name` に人の名前を入れない。** 受領を確定しただけの James が「Reporter」として表示され、**本人が「提出した記憶がない」と答える**事態になった。起票元（受領チェック）を名乗らせ、確定者は本文に書く。
    - ⚠️ **`incident_datetime` に `utcnow()` を入れない。** 7月の納品が9月の事案として記録されていた。**事象が起きた日（納品日）**を入れる。
    - ⚠️ **同じ誤りが集計側にもあった（2026-09-13 発見）。** CK Label Compliance は
      `i.expiry_date < CURRENT_DATE` で「期限切れ」を数えており、**4,348件中2,270件（53%）を
      赤字で EXPIRED と表示**していた。引き渡し時に期限切れだったのは **7件**。
      過去の納品はいつか必ず期限を迎えるので、この式は「古い納品」を全部違反として数える。
      **自動フラグを直したときに、同じ式を読み取り側でも探すこと** — 教訓39の型で、
      53%が赤い画面では本物の7件は見えない。
    - **コードを直しても既存の誤記録は消えない。** 77行の誤フラグと13件のレポートが残る。食品安全の記録を書き換えるのは記録上の判断なので、件数を出して指示を仰ぐこと。

104. **一意制約を足すときは、そのテーブルに INSERT している全部の `ON CONFLICT` を同時に直す — 起動時に走るシードなら、外すのは本番そのもの** → 2026-09-13、統計料率表の重複（`ph_bir_brackets` が6区分で11,790行、`ph_holiday_calendar` が42祝日で10,758行）を消すため一意索引を足した。祝日シードだけ `ON CONFLICT (holiday_date, applies_to_city) DO NOTHING` という**別の対象**を指しており、しかも `applies_to_city` は全行 NULL。一意索引は NULL を互いに異なる値として扱うので、**この対象は最初から一度も一致していなかった**（だから重複が溜まった）。そこへ `(holiday_date, holiday_name)` の索引を足すと、シードは「飛ばす」ではなく **`UniqueViolation` を投げる**ようになり、それが `@app.on_event("startup")` の中だったので **web dyno が起動できず、API全体が約9分停止**した。
    - **対象を書いた `ON CONFLICT` は、その対象の索引が実在するか確認する。** 無ければ節は飾りで、重複は静かに溜まる。`\d テーブル名` か `pg_indexes` で見る。
    - **迷ったら対象を書かない。** 素の `ON CONFLICT DO NOTHING` は「存在するどの一意制約でも」効き、**制約が無ければ何もしない**ので、索引とシードの足並みがずれても落ちない。対象を書くのは `DO UPDATE` が要るときだけ。
    - **索引を落としても直らない。** 起動時のコードが作り直すので、`DROP INDEX` は次の起動で巻き戻る。**止血はコード側**（対象から外してデプロイ）で、DB操作はその後。私はこれを取り違えて復旧を数分遅らせた。
    - **起動フックに移行を足すと、失敗が「その機能が動かない」ではなく「全部が落ちる」になる。** `_run()` で包んである関数は安全だが、`ensure_manila_payroll_tables()` のように**素で呼ばれている**ものは1つの例外でブートを殺す。
    - 教訓95（制約を広げたら `ON CONFLICT` を全部数える）の裏返し。あちらは制約を広げて `ON CONFLICT` が壊れた話、こちらは**制約を新設して壊した**話で、入口は同じ。

105. **半月ごとの源泉徴収を「月給の見込み」から計算しない — BIR は期間ごとの表で、実際に支払った額に対して引く** → マニラの給与は1回目の締めで `staff.monthly_rate` を「その月の総支給の見込み」として渡し、そこから税額を出して半分を引いていた。満額働く人には近い値になるが、**両方向に外れる**。
    - 2026-09-09 入社の Jaybert M. Rivera は2日で ₱2,000.44、そこから **₱232.76** 引かれた。半月表の非課税枠は **₱10,416.67**（年 250,000 ÷ 24）で、彼の課税対象は ₱924.32。**正しくは ₱0。** 手取りは総支給の35%になっていた。
    - 逆に**残業がある人は不足**する。見込みに残業が入らないため。8月は上位14名で **₱2,161.70** 不足。これは従業員ではなく**会社の責任**になる。
    - **年末調整が実装されていない。** どちらのズレも自動では戻らない。
    - 直し方は RR 11-2018 §2.79(B) のとおり、**その期間に実際に支払った額**を期間の表に当てる。半月なら「期間の課税対象 × 24 → 年額区分 → ÷24」で、実装上は `(その期間の総支給 × 2)` を月額として渡せば既存の月額ロジックがそのまま半月表と一致する。
    - ⚠️ **SSS・PhilHealth・Pag-IBIG は別の話。** → 詳細は教訓106。
      **2026-09-13 に私が「Pag-IBIG は現行実装で正しい」と書いたのは誤り。**
      金額はたまたま一致していたが、渡していたのは契約レートで、基礎は実報酬だった。
      **金額が合っていることを、式が合っている根拠にしない。**
    - 逃げ道: `heroku config:set BIR_BASE=monthly_rate`（デプロイ不要で元の挙動に戻る）。**再計算は実際の手取りを動かす**ので、戻り道はリリースを待たない形で持つ。

106. **法定控除の4制度は基礎が違う。「法定控除」とひとくくりにして同じ基礎を当てた瞬間に、月中入社が壊れる** → 2026-09-13、9/9入社・2日勤務・総支給₱2,000.44の Jaybert M. Rivera から **₱1,309（総支給の65%）** が引かれていた。原因は1つではなく3つで、**どれも「4制度は同じもの」という思い込みから来ていた。**

    | 制度 | 正しい基礎 | 根拠 | 月中入社の初回月 |
    |---|---|---|---|
    | SSS | その月の**実報酬** | RA 11199（MSCは "total actual remuneration"） | 実報酬でMSCを決める |
    | WISP/MPF | SSSのMSCが**₱20,000超のときだけ** | 同上 | 実報酬が20,000以下なら**発生しない** |
    | PhilHealth | **契約上の月額基本給** | Advisory 2025-0002（MBSは固定基本レート。OT・手当・賞与を含まない） | **全月分。日割りしない** |
    | Pag-IBIG | その月の**実報酬**（上限₱10,000） | HDMF Circular 460 | 実報酬で計算 |
    | BIR | **その締め期間の実報酬** | RR 11-2018 Annex E | 半月表。₱10,417以下は₱0 |

    - **SSSとPag-IBIGが契約レートを読んでいた。** Jaybertは実報酬が約₱16,000なのに MSC 26,000 で判定され、**本来発生しないWISP ₱150** まで引かれていた（MPF層はMSC 20,000超にしか存在しない）。
    - **PhilHealthだけは契約レートで正しい。** ここが混同の源。**「実報酬に統一」も「レートに統一」も、どちらも間違い。**
    - **各締めが自分のブラケットを引いて50%ずつ取ると、月合計がどの行にも一致しない。** 8月は63名中34名が該当し、**会員記録に計上できない額**になっていた。直し方は「**2回目が精算する**」— 月額から1回目が実際に取った額を引く。取りすぎていたら `REFUND_STATUTORY_OVERDEDUCTION` として返す。これで月合計は必ず表の1行に着地する（実測で確認）。
    - **初回締めの勤務日数が締め期間日数の半分未満なら、その締めでは徴収しない**（`STATUTORY_DEFERRED`）。免除ではなく、**給与のある締めから月額を全部取る**。2日分の給与から半月分を引くのは比例控除ではない。
      ⚠️ **対象は「入社日を含む締め」だけに限る。** 条件を全員に当てると、欠勤の多い月が徴収逃れになる。実データで検証したところ、この規則は現場が挙げた5名にちょうど一致し、10日働いた6人目（Keven Jhon A. Ayubong）は通常どおりになった。
    - ⚠️ **料率がコードに直書きされていないか確認する。** `_compute_philhealth` と `_compute_pagibig` は `conn` を引数に取りながら一度も使わず、5%・₱10,000・2% を**リテラルで持っていた**。料率表を更新しても給与は古い値で計算され続ける。表を読ませ、リテラルは「行が無いときのフォールバック」に落として、**どちらを使ったかを明細のnoteに出す**。
    - **副産物**: 実報酬基準にしたことで、**手取りがマイナス（−₱850.82）の給与明細**が出ていた1名がゼロになった。0円の締めに控除が走っていた。

107. **法定控除を実装したら、年末調整と退職時精算まで含めて1つの機能。締めごとの計算だけでは法令を満たさない** → 2026-09-13、教訓106で4制度の基礎を直したが、**年末調整（NIRC §79(H)）が無い状態**を「未解決点」として報告した。外部レビューで指摘された — それは未解決点ではなく**法令違反の状態**。
    - NIRC §79(H): 暦年末、最終給与の支払い前に、雇用主が**年間の税額を確定し、1〜11月の源泉徴収額との差額を12月給与で精算する**（不足は徴収、過大は返還）。締めごとの源泉徴収は分割前払いにすぎず、半月表から積み上げた額は**所得が動いた人では必ず年額と一致しない**。
    - **退職者も同じ。** 2回目の締めが存在しない人は、最終締めが自分で精算する必要がある。**PhilHealth は退職月も全月分**が必要で、50%しか引かないと差額を雇用主が負担し、加入資格に空白ができる。
    - ⚠️ **年末調整は「システムにある期間」ではなく「暦年」で計算する。** このシステムは
      2026-06-25 以降しか持たない。**そのまま年額を出すと半年分の所得で税額を計算し、全員を
      過小徴収する。** 旧システムの年初来実績を入れる器（`manila_ytd_opening`）が先に要る。
    - ⚠️ **【2026-09-14 訂正】ここには「年が不完全なときは徴収はするが返金しない」と
      書いてあった。それは違法な方針だった。** 私が書き、スタッフへの文面にも入れかけた。
      - **NIRC §79(H) は年末に2つの結末しか認めていない** — 12月給与から差額を徴収するか、
        **翌年1月25日までに返金するか**。「データが揃わないので返さない」という第3の道は無い。
      - **返金しないと罰則がある。NIRC §252**（"Failure of a Withholding Agent to Refund
        Excess Withholding Tax"）は**返金しなかった額と同額**の罰金を定める。
        ⚠️ **§251 ではない** — §251 は徴収・納付の懈怠。RMC 21-2010 が両方を引用しており、
        §255 の刑事責任にも「refund excess taxes withheld」が含まれる。
      - **さらに substituted filing が壊れる。** 条件は ①年内に1社のみ ②**税額＝源泉額**
        （RR 2-98 §2.83.4／RMC 1-2003）。返金しなければ源泉額>税額になり、**店舗スタッフが
        自分で Form 1700 を出さないと取り戻せない。** 彼らは出さない。
      - **正しい対処は「返金を拒む」ではなく「12月の計算より前に年初来データを入れる」。**
        このシステムは 2026-06-25 以降しか持たないので、`manila_ytd_opening`（行数0）を
        埋める。**穴は暦日ではなく給与期間で定義する** — 締めが10日と25日なので、
        「1月1日〜6月24日」と書くと6/11〜6/25の締めで二重計上か欠落が起きる。
      - **元の指摘のうち残る真実は1つだけ**: *途中計算*を不完全なデータで出したときは、
        その数字を安全な向きにだけ使う。**それは年末調整そのものの話ではない。**
        検証で ₱188.95 の誤った返金が出たのは、年初来が空のまま年額を出したからで、
        直すべきは返金の可否ではなく入力だった。
    - 13ヶ月給与・賞与は SSS・PhilHealth・Pag-IBIG・源泉のいずれの基礎からも外れる
      （PD 851 施行規則§6 / SSS Circular 22-P）。**まだ支給していない機能でも、
      支給コードを先に決めて除外リストに入れておく** — `MANUAL_ADDITION` で払われた瞬間に
      基礎へ入る。
    - **自分の整理を外部レビューに掛けると、こちらが「論点」と書いたものが「違反」だと分かる。**
      教訓36で「一次資料に当たれば1時間で決着する論点を、判断待ちとして寝かせない」と書いたが、
      今回は**寝かせてよい論点かどうかの判定自体を誤っていた。**

108. **2人で確かめて「検証済み」と書くな。不変条件は全件で回して初めて検証になる** → 2026-09-13、法定控除の「2回目が精算するので月合計は必ず料率表の1行になる」という設計を、Reymar と Camilla の2名で確認して**「✅ 検証済み」と報告した。** 全61名で回したら**9名が表のどの行とも一致しなかった。**
    - 原因は私の実装。精算額がマイナス（1回目が取りすぎ）のとき、4制度の差額を **`REFUND_STATUTORY_OVERDEDUCTION` という1本の行にまとめて**返していた。制度ごとの控除行は抑止されるので、**その制度の月合計は1回目の額のまま**残る。Abegail A. Dalida は WISP が月内に消えたのに1回目の 12.50 が残り、(950.00, 12.50) という**SSS表に存在しない組み合わせ**になった。会員記録に計上できない。
    - **まとめて返す行は、守るはずの不変条件をそれ自体が壊していた。** 返金は**制度ごとに出す**（`REFUND_SSS_EE` 等）。そうすれば「控除 − 返金 = その制度の月額」が常に成り立ち、他に推論すべきものが無くなる。
    - 同じ全件実行で**手取りマイナス1名**も出た。Aldrin Jay Alowa は2回目を全休し、月額の拠出は正しいのに **₱31.46 の給与から ₱356.74** を引いて **−₱325.28** になった。`_no_compensation` は `G ≤ 0` でしか発火しないので、**「給与が少しだけある」は素通りする。**
    - 直し方は**従業員側を給与の範囲で按分し、取れなかった額を明細に名前で出す**（`STATUTORY_UNCOLLECTED`）。雇用主の納付義務は変わらない。**1回の給与から取れる額の話であって、免除ではない。**
    - ⚠️ **監査スクリプト自体が古くなる。** 返金を制度ごとに変えた後、私の監査は `REFUND_STATUTORY_OVERDEDUCTION` だけを探していて「返金なし」と表示した。**検証コードは検証対象と一緒に直す。** 直さないと「問題なし」が嘘になる。
    - CLAUDE.md には既に「『全員が〜』と書く前に全件を出して目で確認する」とある。**不変条件の検証はその最たるもので、標本2件は標本ですらない。**

109. **人が選ぶ単位と、画面が数える単位を揃える — ずれると「壊れている」に見える** → 面接の空き枠は面接官ごとに数えるので、4人が空いている 09:00 は4件になる。応募者は面接官を選ばないので、予約ページには **「Wednesday 16 September 09:00」が3つ並び、6つの候補のうち本当の選択肢は2つ**だった。どれを押しても予約はできる。壊れているのは見え方だけで、**だから API の往復テストでは絶対に出ない**（教訓56）。
    - ⚠️ **重複を畳むと、逃げ道も一緒に畳まれる。** 時刻で1行にした結果、全員が先頭の面接官を指すようになり、同じ 09:00 を2人が押すと **他に3人空いているのに2人目が「埋まりました」**になった。画面には他の 09:00 が無いので、空いている時刻が埋まって見える。**押した人が選んだのは時刻であって人ではない**ので、繰り上げる。
    - **「N free」は容量、候補は選択肢。** 同じ数字にしようとしない。意味を画面に書く。

110. **凡例が説明している印が、判定条件のせいで一度も出ないことがある** → カレンダーは「シフト未公開の日は `*`」と書きながら、日単位の判定が **「その日の枠が全部推測なら仮」**だった。固定時間の面接官（オーナーが決めた枠＝推測ではない）が平日を全部埋めるので、条件は**成立しようがない**。結果、**10月はシフトが1日も公開されていないのに9月と同じ見た目**だった。
    - **凡例を書いたら、その印が実際に出る日を1つ挙げてから閉じる。** 挙げられなければ条件が間違っている。
    - 同じ画面で、今日の欄が「—」と出ていた。実装上「予約できるのは翌日から」なのに、凡例は「—＝誰も勤務していない」と説明していた。**1つの記号に2つの意味を持たせない。**

111. **予定表に書き出すとき、`LOCATION` に「場所でないもの」を入れない** → 面接の `.ics` は `LOCATION:Office phone`（＝連絡手段）を書いていた。面接は **BO と Cubao の2か所**で行い、Discord DM には「At BO」と書いてあるのに、**携帯のカレンダーに取り込んだ瞬間にその情報だけが落ちる**。
    - **同じものを2か所から書き出せるなら、両方の中身を突き合わせる。** 一覧から作る `.ics` には電話番号が入り、カレンダーから作る方には入っていなかった。**電話面接なので、掛ける番号の無い予定は用をなさない。**

112. **本番データで通し稽古をするなら、外向きの通知を止めるスイッチを先に作る** → 応募者の予約ボタンを実際に押すと、面接官に「予約が入りました」→「取り消されました」と本物の Discord DM が飛ぶ。**スイッチが無いせいで、この一番大事な1クリックだけが検証されないまま残っていた。**
    - `HR_INTERVIEW_NOTIFY=0` で保留してログに出す。**既定は送る。** 検証が終わったら `config:unset` で戻す（消し忘れると本番で誰にも届かない）。
    - 教訓56（最後は必ずボタンを押す）は、**押せる状態を作るところまで**含む。押すと同僚が困るから押さない、は検証しなかったのと同じ。

113. **「記録を作る道」が2つあるなら、下流が読む列を両方が埋めているか確かめる** → 応募者モーダルの「Schedule Interview」は日付・時刻・場所・面接官を手入力させ、**`starts_at` の無い行**を書いていた。面接一覧・カレンダー・前日リマインダー・面接官DMは**全部 `starts_at` を読む**ので、この行はどこにも出ない。その一方で「予約リンク待ち」の一覧は `status IN ('scheduled','held')` だけで除外していたので、**その人は招待できなくなる**。つまり「予定を入れる」の唯一の効果が「招待できなくする」だった。
    - 該当4件は 2026-07〜09 のもので、全員すでに rejected / interviewed。**screened の人に1件付いた日に初めて人が消える**ので、症状が出るまで誰も気づけない形だった。
    - 直し方は2つ同時。**読み取り側**は「時刻の無い枠は予約ではない」として無視する。**書き込み側**は本物の予約と同じ経路に通す（実在する空き枠から選ばせる）。片方だけだと、もう片方から同じ行がまた入る。
    - 盤面の「Interview Sched.」ボタンで同じ型を直した**すぐ後**に、別画面で生き残っていた。**1つ直したら、同じテーブルに書く経路を grep で全部数える**（教訓55・62と同型）。

114. **保存に成功したのに何も言わない画面は、失敗した画面と見分けがつかない** → 面接の日程変更は**新しい枠を作る**ので、保存後の再取得で行の `id` が変わる。確認メッセージを枠の id で持っていたため、**成功したのに「動かしました」がどこにも出なかった**。
    - **確認文は、その操作で変わらない鍵で持つ**（ここでは応募者ID）。行のIDは、その行を作り直す操作では使えない。
    - APIは前後とも200を返す。**押して初めて出る**（教訓56）。

113. **符号を人に打たせている数値欄は、いずれ符号の落ちた記録で埋まる — そして「危険」の色がその typo を事案に見せる** → Travel Path の温度欄は `type="number"` の素の入力で、冷蔵庫か冷凍庫かは**ラベルにしか書かれていなかった**。7月以降の5,206件のうち**677件（13%）が符号の逆側**にある（chiller −8.6℃ / freezer +17.3℃）。
    - **これは設備の故障ではない。記入者で決まる。** 45%の人がいて0%の人がいる。**壊れた冷凍庫は、開けた全員にとって温かい。** 台ごとではなく人ごとに割合が動いたら、入力の問題。
    - 直し方は警告の追加ではなく**符号を打たせないこと**。± ボタンが上下を持ち、既定はラベルの種別から決まる（freezer なら −）。数値欄は `min="0"` でマイナスを受け付けない。**黙って書き換えない** — 符号は画面に出ていて1タップで変えられる。
    - **符号の落ちた値を赤にしてはいけない。** 赤は「食品が危険だった」と言う。事実は「この読みは使えない」。**typo を事案として数えると、本物の事案が埋まる**（教訓39）。琥珀色＋理由を書く。
    - ⚠️ **不良の連続回数を数えるときは、読めない値を先に捨てる。** 捨てる前は TAFT Freezer 2 が「77回連続」、捨てると「56回連続」。**どちらも本物だが、捨てないと本物と typo が同じ山に入るので、閾値を決められない。** 実測で連続3回以上は**3台だけ**（TAFT F2/F1・PAR F1）で、生の「危険」654件とは桁が違う。
    - 未了: この3台は**8月から一度も −18℃ に戻っていない**のに、どこにも通知が飛ばない。`getTempStatus()` は画面の色だけで、worker も Discord も無い。677件の既存記録は食品安全の記録なので書き換えていない。

115. **応募者の手元にあるリンクは、切れた瞬間に本人が詰む — 期限は1か所で決め、切れる前に画面へ出す** → 有効日数が**6か所にリテラルで書かれ**、2種類に割れていた（`create_screening`=7 / `issue_invite`=14 / 応募フォームが渡す7 / エンドポイント2本が渡す14 / 予約リンク=7）。誰も選んでいない、溜まっただけの差。2026-09-15、7日側が一斉に満期を迎え、**9/8発行の音声リンク10本が同じ午後に30分おきに切れた**（13:27〜17:02）。応募者から「リンクが切れている」と連絡が来て発覚。
    - **切れたリンクは修復できない。** トークンはハッシュしか保存していないので、**古いメッセージを転送しても無意味**（中身が死んだリンク）。再発行だけが直し方で、それは応募者に2通目を送ることになる。
    - **既定値に数字を書かない。** 既定引数は import 時に固定されるので、2種類が生き延びた原因そのもの。`days_valid: Optional[int] = None` にして、**呼び出し時に** `applicant_links.link_days()` を読む（教訓61）。`APPLICANT_LINK_DAYS` でデプロイ不要に動く。
    - ⚠️ **画面は `token_expires_at` を受け取っていたのに、どこからも見ていなかった。** 期限切れの10人は「never opened the link」と表示され、**遅いのは応募者だ、と読める形**になっていた。教訓58と同型（「やることなし」と「見えていない」を同じ表示にしない）。
    - **事後の表示だけでは足りない。** 「切れた」を出すのは、既に詰んだ人の報告。**残り3日以下でカウントダウンを出す**（全行に数字を出すと誰も読まない／教訓39）。
    - 既に配ってある分は**延長で揃う**。期限を後ろに動かすだけでトークンは無効にならないので、生きている156本（音声125・予約31）を14日に揃えた。**`GREATEST` で短くする方向には動かさない。** 退避は `_voice_expiry_bk_20260915` / `_booking_expiry_bk_20260915`。

116. **`fixed inset-0 flex items-center justify-center` のモーダルは、スマホで入力できない** → 2026-09-16、マニラのHRから「面接結果のコメント欄で**1文字打つと画面から抜ける。スペースを押すと枠から出て下にスクロールする**」と報告。**マウスでは再現しない。小さい画面だけで出る。** 症状は2つに見えて原因は1つ。
    - **背後のページがロックされていない。** 採用ボードは **11,508px** ある。モーダルが消費しなかったスクロールは全部そちらが動く。**フォーカスが入力欄の外にあるとスペースはページをスクロールする** ── これが「枠から出て下にスクロール」の正体。
    - **溢れている子を `align-items: center` で中央寄せすると、上端が親の外に出て届かない。** 面接結果モーダルは **729px の枠に 921px** 必要だった。そこにソフトキーボードが出てビューポートが縮み、ブラウザがフォーカス中の欄を見せようとすると、置き場所が無い。**固定レイヤーごとずれて「画面から抜けた」ように見える。**
    - 直し方: **スクリム自身を通常のブロックフローでスクロールさせ**、子は `mx-auto my-4` で中央に置く。子から `max-h-[90vh] overflow-y-auto` を外す。`overscroll-contain` で端を越えたドラッグがページスクロールやプルリフレッシュにならないようにする。**そして body をロックする。**
    - ⚠️ **ロックは各モーダルに書かない。忘れるのがそこだから。** `src/components/ModalScrim.tsx` に置き、スクリムをマウントする以外に背景を作れない形にした。
    - **同じ形が 2026-09-15 に Pay Slip でも報告されている**（「上にスクロールしようとすると前のページに戻る」）。同じ原因・同じ直し方。
    - ⚠️ **残り66ファイルが同じパターンを持っている。** 報告のあった画面（HR Recruitment の8モーダル）だけ直した。一括置換はJSXの閉じタグを機械的に対応付ける必要があり（教訓1）、自己終了 `<div ... />` を開始タグとして数えると全部ずれる。
    - 検証はモバイル幅で実際に打つこと。`document.activeElement` と `window.scrollY` を打鍵の前後で見れば、フォーカスが外れたのかページが動いたのかが分かる。

117. **承認者が「申請とは違う結論」を出したとき、それを書く欄が自由記述しか無ければ、下流は申請どおり支払う** → 2026-09-16、マニラから「2時間のOT申請に1時間だけ承認したケースが数回あり、コメント欄にメモとして記載していた。支払い処理時の見落としにつながる」と報告。**既に起きていた。** Ricardo Lamis III 2026-09-09 は `manager_note='Please approve 1hr. Thank you.'` を持ちながら `ot_minutes=120` のままで、**DTR には 2.00 時間が入っている。** 同型が4件（うちドバイ2件は 600分に「1hr approved」、663分に「2hrs approved」）。
    - **「承認」がフラグ1つしか持たないと、部分承認は行き場を失う。** 承認は yes/no ではなく**金額を伴う判断**なのに、書き込める場所が status だけだった。人は必ず空いている欄（コメント）に書く。**そして機械は列を読む。**
    - **画面上は矛盾して見えない。** 行には申請どおりの分数とコメントが並ぶので、読めば分かる。**読まないのは給与の同期処理**で、そこには目が無い。教訓48（1つの列に複数の意味）の裏返しで、こちらは**意味が列の外に置かれた**形。
    - 直し方は欄を足すこと（`ot_minutes_reason`）と、**理由を必須にすること**。打刻から計算した分数（`source='clock'`）は根拠が画面にあるので理由は要らないが、**人が打った分数は理由が唯一の根拠**なので必須にした。この2つを同じ扱いにしない。
    - ⚠️ **既存の不一致は、コードを直しても消えない。** 4件は手で直すか、そのままにするかの判断が要る。**「機能を作った」と「記録が正しくなった」は別。**
    - 探し方: 自由記述の列を、**下流が読む列と突き合わせる**。`manager_note ~* '\m\d+\s*(hr|hour)'` のような検索で、数量を書いたノートを持つ行を出し、その行の数量列と比べる。

123. **頼まれていない上限を足すと、頼まれたルールが別のルールに置き換わる** → 面接の割り当ては「Peter ができる時は Peter、できない時に次の人」という指示だった。私はそれを実装したうえで、**誰にも頼まれていない「1日4件まで」**を既定値として入れた（`HR_INTERVIEW_MAX_PER_DAY`、コミット `c7a24b83`）。結果、動いていたのは
    > 「Peter が **4件やった** 時に次の人」
    - **9/18 が実例**: Peter は BO 9:00–18:00 に公開シフトがあり 11:30・15:30・16:30 は空いていたのに、4件目の後だったので3件とも Camilla に回った。全体では Peter 23 / Camilla 10。**オーナーには「繰り上げが効いていない」と見える。** 効いていたのは繰り上げではなく上限。
    - ⚠️ **私は最初、データを見て「繰り上げは動いています」と報告しかけた。** Camilla にも配分されていたからで、それは上限の作動であって指示の実現ではなかった。**「指示どおりか」は、分布ではなく〈下位に回った1件ごとに、上位が本当にできなかったか〉で確かめる。** 7/34 がそれで炙り出せた。
    - **既定値は仕様である。** `_env_int("X", 4)` の 4 は、設定していない全員にとっての仕様そのもの。**自分で決めた既定値には、決めたのが自分だと書く** — コメントに根拠が無いものは、後から読むと「そう決まっている」に見える。
    - 上限そのものは消さず、**既定を「無し」にして設定で戻せる形**にした。要ると分かったときにコードを触らなくてよい。
    - 同じファイルで、オーナー指示と書いてある制約（同時刻は面接官をまたいで1件・月〜金のみ）は残した。**出所が書いてある制約と、書いていない制約を分けられることが、この直しを1分で終わらせた。**


122. **自由入力の値を `href` にそのまま入れると、URLでないものは自社ドメインの相対パスになって404になる** → 2026-09-16、「Facebookのリンクが開けない」と報告。`href={applicant.facebook_url}` に生の値を入れており、`Jonas Membrillos` が `/admin/hr/Jonas%20Membrillos` に解決されていた。**エラーは出ない。リンクに見えて、押すと自社の404が出る。**
    - 実測: 記入済み120件のうち**開いたのは33件（完全なURL）だけ**。氏名54・ハンドルのみ21・スキーマ無し `facebook.com/…` 12 の**計87件（73%）**が、この項目ができてからずっと開かなかった。
    - ⚠️ **フォームは「リンク、または名前」と聞いていた。** 入力側が両方を許しているのに、表示側がURLだと決めていた。**片方だけ読むと、この不一致は見えない。**
    - **氏名からプロフィールURLは作れない。** 当てずっぽうに `facebook.com/<名前>` を組むと同じ404になる。**検索に落とすのが正しい** ── 人が手でやるのもそれ。そして**検索であることを画面に書く**（検索結果を本人と読み違えると、別人の情報で採用判断をする）。
    - **ハンドルらしき文字列を全部URLにしない。** `-` / `facebook` / `조니콜` / メールアドレスは、URLにすれば404になるだけ。**死んだプロフィールURLは、直したはずの死んだリンクと同じ。**
    - 検証は**実データ全件を関数に通す**。`tests/facebook-link.test.ts` が本番の120件を回して、相対hrefが1件も出ないことを見ている。考えた例だけでテストすると、`Facebook.com/jeorgina cainan`（パスに空白）や `@nickimperial.mariano`（先頭@をメール扱いして落とす）は出てこない ── 後者は実際に私の最初の実装が落とした。


121. **「生体認証に変えたい」と言われたら、まず既にあるかを確かめる。無いのは認証方式ではなく、既定値を拒む判定であることが多い** → 2026-09-16、「My Pay が初期PIN 1111 で見えてしまうので指紋・顔認証にしたい」。**パスキー（WebAuthn）は既に実装済みで、147名・471件が登録済みだった。** 画面にも `Verify with Passkey` が出ている。穴は隣の「Use PIN instead」で、**177アカウント中157が 1111 のまま**（在籍109）。作るべきだったのは認証方式ではなく、**正しく入力された既定PINを拒む判定**。
    - **PINはハッシュなので「誰が1111か」はクエリでは出ない。** 氏名ごとに `verify_staff_pin(name, "1111")` を回して数える。これをやるまで規模が分からない。
    - ⚠️ **その PIN はログインPINと同じだった**（`verify_staff_pin` が `/api/auth/verify` の判定）。**依頼は My Pay の話だが、穴は My Pay に閉じていない。** 依頼された画面だけ直して報告すると、はるかに大きい露出が残る。
    - **既定値の判定は狭く作る。** 配布値・同一数字・繰り返し・連番まで。「人が本当に選びそうなもの」まで拒むリストは、PINをカードの裏に書かせるので、止めたPINより悪い。1357・2580・8341 は通ること実測。
    - **新しいPINを決める経路すべてに同じ判定を入れる。** `change_pin` と初回セットアップの2箇所あり、片方だけだと全員が 0000 に移るだけ（教訓62と同型）。
    - **拒否は行き止まりにしない。** 「PINが違う」ではなく「合っているが配布時のもの」と書き、**その画面から**指紋登録とPIN変更の両方に行けるようにする。パスキー登録が別ページにしか無かったことが、「Use PIN instead」が最短経路であり続けた理由。
    - **既存157件のPINを一斉に変えるのは運用イベント**（在籍109名）。コードの穴を塞ぐことと、配られた秘密を回収することは別の作業で、後者はオーナーの判断。
    - ⚠️ **続き（同日、パスキー必須化）: 認証方式を絞るなら、画面ではなくAPIで閉じる。** ステップアップトークンには発行方法（`method`）が残るので、給与明細のエンドポイントで `method == 'passkey'` を要求する。**ボタンを消すだけでは、header を送れば通る。**
    - **絞る前に「登録に何が要るか」を確認する。** `webauthn/register/options` はログインだけで通る（既存パスキー不要）ので、端末紛失でも機種変更でも自分で登録し直せる ＝ 詰まない。**もし登録にステップアップが要る設計だったら、パスキー必須は「パスキーが無いとパスキーを作れない」デッドロックになっていた。**
    - **到達不能になったコードは消す。** PIN パネルを残したままボタンだけ外すと、次に読む人が「使えるはず」と考える。
    - ⚠️ **その画面のテストが、消した経路でゲートを通っていた。** My Pay のテスト69件が「Use PIN instead」をクリックしており、ボタンを消した瞬間に全滅した。**認証の入口を変えるときは、その入口を使っているテストを先に数える。** jsdom には authenticator が無いので `navigator.credentials` と `AuthenticatorAssertionResponse` / `AuthenticatorAttestationResponse` を与える必要がある（後者はページが `instanceof` で分岐しており、未定義だと名前解決で落ちる）。


120. **提出時に1回だけ書かれる「等級」が、下流の自動処理の入口を決めているなら、それは変更できなければならない** → 2026-09-16、Camilla の勤怠インシデント（Taft の複数欠勤）が `severity='medium'` で提出された。オーナーの問い —「これは提出した人しか変えられないのか」。**誰も変えられなかった。** `incident_reports.severity` を UPDATE するコードは全コードベースに存在せず、提出時の1回きりで、以後は読むだけ。
    - **これは見た目の話ではない。** 未対応レポートを追いかける `get_urgent_reports_to_notify` は **`severity IN ('high','critical')` の行しか拾わない**（critical=6時間ごと / high=24時間ごと）。**medium は一度も追いかけられない。** つまり等級を1段下げて出すと、フォローアップの仕組みから丸ごと外れる。**提出者が、自分で気づかないまま、その機能を無効化できる状態だった。**
    - 直し方は `PATCH /api/admin/incidents/{id}/severity`。権限は返信・ステータス変更と同じ（＝そのレポートを扱う人）。**提出時の等級を `severity_original` に残す** — 現場の人がその場でどう読んだかは、後で較正するときに必要な情報だから。理由必須。**提出者に通知**する（他人の判断について判断しているので、黙って変えない）。
    - ⚠️ **同じ画面で、同じ出来事の時刻が2つ、4時間ずれて出ていた。** ヘッダ 08:54 / 事案欄 12:53。`fmtDt` が `new Date(iso).toLocaleString()` で**読む人の端末のタイムゾーン**で描いており、読んでいたのがドバイ時刻のPCだった。`timeZone: <支店の都市>` を明示する。**`incident_datetime` は `datetime-local` の素の文字列（＝既に現地の壁時計）なので、Date に通してはいけない** — 人が打った時刻が動く。CLAUDE.md 冒頭の表にある型の再演。
    - **語彙は1か所。** `SEVERITY_LEVELS`（API層）と等級の一覧が二重にあった。追いかけの条件がこの語で書かれている以上、2つ持つと必ずずれる。


119. **「未処理だけ」を出す画面は、他人が代わりにやった瞬間に何も言わなくなる — そして空状態が「何も無かった」と断言する** → 2026-09-16、マニラから「昨日の Quality C のアラートが Management Box に来ているが、今朝の Morning Review に反映されていない」と報告。**反映されていた。** 05:00 に生成され、彼宛で、C 2件が明細として乗っていた。**11:43 に Yuri が2件とも回答して完了させた。** 画面は `status=open` しか要求しないので、その瞬間に彼の一覧から消え、空状態が「レビューはその日に何かあった日にだけ作られます」と表示した。結果、彼は同じ2件を Management Inbox から二重に処理した。
    - **代行は例外ではない。** 完了65件のうち**32件が担当者以外による完了**（一括クローズ script の21件を除いても 11/44）。Yuri と Yusuke は日常的に互いを代行している。
    - ⚠️ **`_ops_review_may_touch` の docstring は「完了7件中、担当者以外による完了は0件。誰も通らない扉を閉めるだけ」と書いてあった。** 書いた時点では真で、**その数字を根拠に権限を決めていた**。いまは 32/65。**根拠にした実測値は、時間が経つと黙って嘘になる。** 権限の判断を数字で正当化したなら、その数字を後で測り直す。
    - 直し方は `done_for_me()` ── 直近1週間に**他人が自分の名前で回答した**レビューを別枠で出す。**「誰かが完了させた」だけでは出さない**: 中身の無い日や、一括クローズが閉じただけの行を並べると20行のノイズになり、本物1行が埋まる（教訓39）。実際 `HAVING` を足す前は Yuri の画面に script の20行が並んだ。全社で残るのは2行で、どちらも本物。
    - **完了済みを開けるようにしたら、同時に読み取り専用にする。** それまで完了済みは一覧に出なかったので誰も守っていなかった。開いた瞬間に相手の回答を上書きできる状態だった。
    - ⚠️ **`completed_at::text` はDBのタイムゾーン（UTC）で出る。** 担当者に「03:44」と表示していた（実際はマニラ 11:44）。`to_char(... AT TIME ZONE ...)` で店舗の時計に直す。


118. **1人に1枠しかない秘密を「送る唯一の手段」にすると、送るたびに相手の手元を壊す** → 予約リンクは `hr_applicants.booking_token_hash` という**1つの列**に貼り替えていた。トークンは生で保存しないので「前のリンクをもう一度」は不可能 ── つまり**発行が送信の唯一の道**で、押すたびに相手が既に持っているリンクが死ぬ。2026-09-16、応募者から「The link is not working」と返信が来て発覚。彼女の記録には **9/29まで有効なリンクがあった**。切れていたのではなく、上書きされていた。
    - 実測: 9/15 に **84名へ117回発行、送信は76回**。差の41回は「開いたが送らなかった」押下で、その1回1回が既に配ったリンクを消しうる。イベント履歴で確定できた被害は1名、**もう1名（報告者）は `booking_sent` が記録されているのに本人のWhatsAppに届いていなかった** — `sent` は貼り付けた後に人が押すボタンなので、**送信の証拠ではない**。
    - 直し方は「警告を出す」ではなく**上書きをやめる**こと。トークンを行にして（`hr_booking_tokens`）、期限内のものは全部生かす。2本目を作っても1本目が壊れないので、**確認ダイアログごと不要になった**（あの文言は、この変更で嘘になる）。
    - ⚠️ **移行時に、いま生きているリンクを必ず運ぶ。** 列にあった84件を行へコピーしてからでないと、デプロイした瞬間に全員が締め出される。**守るために作った変更が、最大の事故になりうる。**
    - **失敗の理由を1つの文言にまとめない。** 「This link has expired」は4つの原因すべてに出ていた（期限切れ／貼り替え／`merged_into` で統合／未知）。**実際に起きたのは2番目**で、応募者は「既に持っている新しいリンク」を再送依頼し、調べる側は期限を見に行った（前日に期限の退避テーブルまで作っていた）。行を残せば理由が言える。
    - ⚠️ **ハッシュしか無いので、変更前に死んだトークンは「未知」としか答えられない。** 貼り替えとタイプミスを区別できない。だから未知の文言は**多い方の原因（貼り替え）に先に答える**。
    - **`opened` を記録する。** こちら側で `sent` は自己申告だが、開封はサーバが見た事実。「送信済みなのに翌日も未開封」が、届かなかった行の見え方。
    - ⚠️ **検証で「最古の行」を期限切れにして、移行したばかりの実トークンを壊した（私の操作）。** 4分で戻したが、教訓54と同型 — **本番で期限を動かすなら、動かす行を先に表示して自分が作った行だと確認する。**
...
125. **「その画面が開ける」と「その画面でその作業ができる」は別の主張 — 案内する前に後者を確かめる** → 2026-09-26、アラートが届かない3名について「Discord ID を登録してください」の案内先を**3回間違えた**。
    ...
    - 2回目: メニューにある `/admin/discord-alerts` を検討 → 書く先は `discord_alert_recipients`＝**店舗別の遅刻アラート名簿**。足すと頼まれていない勤怠DMが届き始める。
    - 3回目: 正しい表を書く `/admin/management/assignments` にリンクし、**リンク先が開くことをブラウザで確認した。そこで止めたのが誤り** — ID入力欄は**例外タイプの担当者にしか描画されない**ので、担当者でない Mariano と Aliana は**その画面では登録できない**。
    - 結局、**当の画面（Direct Purchase）に名前ごとの入力欄と Save を置いて完結させた。** 他所へ送らなければ、経路の存在もメニュー掲載も問題にならない。
    - **確かめ方は「開くか」ではなく「目的の対象が操作できる状態で出ているか」。** 教訓56（最後は必ずボタンを押す）の対象を、自分ではなく**案内される側**に広げたもの。
    - ⚠️ **リンクは文章の案内より安全だが、安全になるのは「辿れない」問題だけ。** 中身が合っていなければ同じ行き止まりになる。

126. **`app/main.py` は調達ブロックを2回登録している — 後ろのコピーは到達不能。直すなら前を直す** → `(method, path)` の重複登録が**16組**ある（例: `GET /api/admin/procurement/vendors` が 26682 と 33788、`POST /api/admin/procurement/cases/{case_id}/reject` が 27686 と 34098）。**FastAPI は最初の登録を採用する**（インストール済みの版で実測確認済み）。
    - **後ろのコピーに入れた修正は永久に動かない。** 本文がほぼ同一なので、grep で最初に当たった方を直すと五分五分で外れる。
    - 症状が出ない形の事故になる:「直した・テストも書いた」のに本番の挙動が変わらない。
    - **同じ状態を書く経路が複数ある機能に手を入れるときは、書き込み箇所を全部列挙し、1つずつ「生きている登録か」を判定する。** 判定は `@app.<method>("<path>")` を全部集め、**同じキーの最小行番号かどうか**を見る。
    - `tests/test_procurement_alerts.py::test_every_reachable_rejection_path_alerts_the_creator` がこの判定を機械化してある（生きている側に全部・死んでいる側に1つも無いことを検査し、4つ目の経路が増えたら落ちる）。同型の機能を作るときはこの検査ごと写す。
    - ⚠️ **重複そのものは直していない**（1,000行規模の削除で影響範囲が別件）。**到達不能なコードが残っている前提で読むこと。**

127. **権限を「1つの都市」で検証して、データを「全都市」で返していないか — 同じ値を2回正規化した時点で必ずずれる** → 2026-09-26、`GET /api/admin/procurement/request-alerts` は `target_city=(city or "manila")` で**マニラとして認可**し、その直後に `c = (city or "").strip().lower()` を別に作って3つのクエリへ渡していた。**`city` が空だと `c` も空になり、全クエリの都市フィルタが無効化される。** 結果、マニラの権限でドバイの作成者名・金額・**却下理由の全文**が返る。
    - **片方は `or "manila"` でフォールバックし、もう片方はしない。** これが原因の全て。**1回だけ解決し、認可にも読み取りにも同じ変数を使う。**
    - 同型の修正が**同じファイルの5,500行手前に既にあり、コメントまで残っていた**。「この型は前に直した」は、次の箇所を直した保証にならない。
    - 検査は機械化した（`target_city=city_req` と `c = city_req` の両方を要求し、2つ目の正規化が復活したら落ちる）。

128. **GET に書き込みを混ぜない — 「1回だけの確認」は毎ページ読み込みで走る** → 書き込み経路が全てPINの後ろにあるので、`probe_record_insert()`（本物の INSERT → ROLLBACK）を読み取りエンドポイントに置いた。そのエンドポイントは**画面を開くたび・フィルタを変えるたび**に呼ばれるので、**1ページビューごとにトランザクションと BIGSERIAL を消費**していた。`?probe=1` のオプトインにした。
    - **「診断のつもりで足したもの」が既定で常時動く仕様になっていないか確認する**（教訓123と同型 — 自分で決めた既定値が仕様になる）。

129. **送ってから記録すると、記録が失われた分だけ二重送信になる** → アラートは「記録してから配信する」と docstring に2箇所書きながら、コードは**送信→記録**の順だった。間でプロセスが落ちると **DMは届いたのに行が無い**状態になり、翌朝の掃引の `NOT EXISTS` 重複排除が効かず**同じアラートをもう一度送る**。
    - 直し方は順序を戻すだけでなく、**中間状態を持たせる**こと。先に `sending` で書き、結果が出たら上書きする。**行が `sending` のままなら「届いたか分からない」**という、成功とも失敗とも違う事実が残る。
    - ⚠️ **外部HTTPをリクエスト経路の中で無制限に待たない。** `send_discord_dm` は10秒×2回。却下エンドポイントの中で待つと Heroku の30秒を超え、**却下は成功しているのにリクエストは失敗して見える** → 担当者がもう一度押す → 承認アクションとDMが二重になる。待ち時間に上限を置き、諦めたことを記録する。
    - ⚠️ **docstring が「こうしている」と書いていても、コードがそうしている保証はない。** この2つの docstring は**最初から嘘**で、レビューされるまで誰も突き合わせなかった。
